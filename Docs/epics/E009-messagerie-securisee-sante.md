...
| done-task-036-fix-version-navigation-routing | **Bug fix sur task-015c — navigation cliquable du badge REMPLACÉ et lien Version précédente**. La US 015c (mergée plus tôt 2026-05-08) avait expédié le code avec `NavigationManager.NavigateTo("/Mail/{uid}")` côté Blazor et `Router.navigate(['/mail', uid])` côté Angular — deux routes inexistantes (Blazor expose `@page "/emails"`, Angular `MSS_ROUTES` n'a que `mail` / `mail/tag/:tagCode`). Erreur Angular observée par le humain : `NG04002: Cannot match any routes. URL Segment: 'mail/4190'`. Cause profonde : la sélection d'un mail dans cette app passe par un **service d'événements**, pas par le routeur. **Blazor** : `MailHeader.razor` injecte désormais `IFolderEventService` (au lieu de `NavigationManager`) et appelle `SelectMailByIdAsync(successor.MailUid)` — pattern utilisé par `MailListComponent` ligne 360 (déjà existant). **Angular** (mode code-only, uncommitted sur `feature/nova-rewriting-mss-fixes-20260410`) : nouveau Subject `MailEventService.requestSelectByUid$` (payload `{ uid, folderPath }`) ; `mail-header.onSupersededBadgeClick` et `mail-detail.openPreviousVersion` émettent dessus au lieu d'utiliser `Router.navigate` ; `MailListComponent.ngOnInit` souscrit, fetch le `MailDto` cible via `api.getEmails(folderPath, [uid])` puis ré-émet sur `mailSelected$` (le pattern in-app pour sélectionner un mail). **Tests durcis** : les bUnit / Vitest existants n'avaient pas attrapé le bug parce qu'ils mockaient NavigationManager / Router et asseraient seulement l'invocation du fetch. La US ajoute des assertions sur le **résultat** (mail sélectionné) — `MailHeaderSupersededBadgeTests` +2 tests (selects successor via folder event service, does not invoke on null successor) ; `mail-header.component.spec` +1 test (does not dispatch on null successor) ; `mail-detail.component.spec` +2 tests sur `openPreviousVersion` (emit + early-return) ; `mail-list.component.spec` (nouveau fichier) +2 tests sur le subscriber `requestSelectByUid$`. Suites globales : Blazor 43 → **45 bUnit verts** (+2) ; Angular mss-lib mail-header 13 → 11 (consolidation API tests, count plat car remplacement spy plutôt qu'ajout brut) ; mail-detail 14 → **16 verts** (+2). **0 changement DTO, 0 changement endpoint, 0 changement DB** — purement frontend. Branche dtos-mss + api-mail créées par `/start` mais 0 commit, pas de PR. Diagnostic du bug effectué via inspection directe Postgres (psql via docker-exec sur container `postgres-pgvector` 5432) : confirmé que la donnée DB du test du humain (Uids 4190/4191) était saine — `SupersededByDocumentId` correctement posé, aucun `DuplicateOfId` parasite. La fausse impression de "détection de doublon" venait d'un test antérieur à 14:37 (Uid 4189) où le humain avait sans le savoir attaché la même version de CDA → verdict Exact légitime mais sémantiquement piégeur. Une US séparée `task-037` (deferred par décision humaine) couvrirait l'option D — header SMTP `X-Cancel-Replaces` pour distinguer explicitement le geste annule-et-remplace des heuristiques INT.18/015a/duplicate. | Bug fix sur 015c — la navigation cliquable est désormais réellement opérationnelle |
...
=======
| done-task-033 | **Cleanup Sonar massif api-mail** — cycle de cleanup automatisé, 1 itération (fixes mécaniques safe épuisés après iter 1). Code smells 166 → 124 (-42, -25%), security hotspots 6 → 5 (-1), ratings A/A/A maintenus. 18 règles traitées sur 38 fichiers : CA1861 ×19 (constant arrays → static readonly), CA1822 ×6 (methods → static), xUnit2032 ×5, CA2254 ×4 (log template → const), SYSLIB1045 ×3 (GeneratedRegex), xUnit2024 ×3, xUnit2025 ×2, S125 ×3, CA1846 ×2 (AsSpan), CA1850 ×1 (SHA256.HashData), CA1859 ×2, CA1866/S6610 ×2, S1075 ×1, S6617 ×1, CA2016 ×1, ASP0025 ×1, S1172 ×1, S2139 ×1. Skippés avec justification : S3776 ×39 (blacklist), CA1862 ×30 (EF LINQ risque DB), S1192 ×22 (migrations + sémantique), S107 ×9 (design), S6960 ×3 (design). Suite api-mail : 1950 passés / 0 failed. Dépendait de task-032 (couverture TU comme oracle de non-régression). | — (dette technique Sonar, prépare la base pour les futures US ; pas un item Ségur) |
>>>>>>> Stashed changes
| done-task-032ter-greenmail-fixture | **Mock IMAP server — Approche A : extraction interface `IMailClientSessionManager`** — Chantier 1 du harness task-032bis-test-harness, livré après découpage Option B 2026-05-07. Extraction de `IMailClientSessionManager` (19 membres publics) depuis `MailClientSessionManager` concrète. 5 consumers mis à jour vers l'interface : `ImapService`, `ImapFolderService`, `ImapConnectionService`, `BackgroundSyncManager`, `MailExportService`. `ImapLockScope` mis à jour (4 refs concrete→interface). DI : `AddSingleton<MailClientSessionManager>()` → `AddSingleton<IMailClientSessionManager, MailClientSessionManager>()`. `MarkLockAcquired`/`MarkLockReleased` promus `internal → public` (requis par `ImapLockScope` via interface). Retrait `[ExcludeFromCodeCoverage]` sur ImapService (1914 LOC), ImapFolderService (612 LOC), ImapConnectionService (310 LOC) — ~2836 LOC réintégrées au dénominateur. `BackgroundImapService` conserve l'exclusion (utilise `ImapClient` directement — GreenMail/Approach B requis). Tests existants (57 IMAP) mis à jour : `Substitute.For<IMailClientSessionManager>()` avec `AcquireLockAsync` délégué à `ImapLockScope.AcquireAsync`. Suite api-mail : 1950 passés / 0 failed. **US-sœurs** : `task-032bis-fhir-mock` (Chantier 2, done), `task-032quater-cda-samples` (Chantier 3, done). | — (outillage qualité / interface extraction IMAP ; pas un item Ségur) |
...
| done-task-035 | **Widget Patient (5 derniers patients avec mails non lus)** — nouveau widget vertical sur le dashboard MSS proposant un accès rapide aux patients ayant ≥ 1 mail non lu, classés par date du mail non-lu le plus récent. **1 patient = 1 ligne agrégée** : avatar à initiales, identité (nom + âge + genre), compteur `N mails non lus`, chips catégories CDA distinctes (Bio / Imagerie / Consultation / etc.), badge sévérité biologie (🔴 critique HL7 `LL/HH/AA`, 🟠 anormal `IsFlagged`), pastille intégration (✓ tous intégrés / ⏳ N en attente — réutilise la logique task-010), badge `DOUBLON` si au moins un mail non-lu marqué doublon (task-013). **Bouton "Voir plus"** étend la liste de 5 → 20. **Menu kebab à 3 actions** : (a) Voir le dossier patient — désactivée avec tooltip si `HasQualifiedIns == false` ; (b) Filtrer mails sur ce patient ; (c) Voir l'email — réutilise `MailDetailComponent` existant via la route `/mail/{uid}` qui déclenche déjà mark-as-read + audit `MailRead` + refresh widget (aucun nouveau composant de visualisation email créé — DOD respecté + grep validé). **Backend** : DTO `PatientWithUnreadMailsDto` + enums `BiologySeverity` (None / Abnormal / Critical) et `IntegrationStatus` (AllIntegrated / PartiallyPending) ajoutés à `dtos-mss` (NuGet **279.0.0** publié) ; endpoint `GET /api/v1/Patients/with-unread-mails?limit={int}` (clamping `[1, 20]`, défaut 5) sur `PatientsController` ; `IPatientService.GetWithUnreadMailsAsync` + `IPatientRepository.GetWithUnreadMailsAsync` — 4 requêtes EF Core bornées (`AsNoTracking`) : unread INBOX docs (filtre `Mail.IsRead == false && FolderPath == "inbox"` + exclusions doublons rejetés / superseded / suppression acceptée), biologies, patients, mails-with-orphans (pour `IntegrationStatus`) — agrégation in-memory et `Take(limit)` final. **Cloisonnement par UserId** : architecturalement garanti via la base PostgreSQL par utilisateur (`BaseRepository.CreateDbContextAsync` construit le DbContext depuis `UserContextInfo.ConnectionStringUser`) — convention task-023 conservée. **Temps réel** : abonnement `IMailHubService.OnEmailsEnriched` + `OnNotificationReceived` côté Blazor, `MailEventsStreamService.emailsEnriched$` côté Angular ; pas de polling. **Frontends** : Blazor `PatientWidgetComponent.razor` auto-discovered comme `IAlertWidget` (registration via reflection — `ModuleLoader.AlertWidgets`) ; Angular `PatientWidgetComponent` standalone signal-based (`changeDetection: OnPush`, `takeUntilDestroyed`) ajouté à `mss-dashboard` (mode code-only — humain gère commit/push TFS sur la branche `feature/nova-rewriting-mss-fixes-20260410`). 14 nouvelles clés Localizer FR + EN côté Blazor ; le module Angular reste hardcoded FR (cohérent avec `AbnormalBiologyWidget` Angular existant — pas d'infrastructure i18n dans `mss-lib`, pré-existing limitation). **Tests** : api-mail **+5 unit `PatientServiceTests` + 6 integration `PatientRepositoryIntegrationTests`** (nominal, empty, limit clamp, exclusion read mails, exclusion docs supprimés, severity critique HL7, limit-zero, cross-tenant deux `IPatientRepository` mockés sur deux services distincts) ; Blazor **+5 bUnit `PatientWidgetComponentTests`** (empty, list+kebab+counter, 3 actions ouvertes, view-patient désactivé sans INS, See More CTA) ; Angular mss-lib **+8 Vitest `patient-widget.component.spec.ts`** (empty, list, kebab, view-patient disabled, See More, expand-to-20, refresh sur enrichissement, custom event `mss-patient-widget-view-email`). Suite api-mail : **1972 passed / 16 skipped (AI tests intentionnels) / 0 failed** (+11 vs baseline post-task-036). Blazor 50/50, Angular mss-lib (patient-widget.spec) 8/8. **`/sonar` best-effort** : 0 itération appliquée — analyse honnête (CA1873 622 issues sur trop de fichiers, S3776 39 blacklisted, CA1862 36 false-positive en EF Core LINQ context, S1192 17 dans migration figée, S107/S6960 refacto signatures DI risquées, hotspots = revue humaine). 3/4 hard targets déjà atteints (`bugs=0`, `vulnerabilities=0`, `sqale_rating=A`) ; coverage à 54.4 % reste long-terme (hors scope d'une itération unique). PRs `awaiting-human-merge` : `dtos-mss#21`, `api-mail#54`, `client-blazor#50`. | — (pas de `**Closes RG**` déclaré ; contribue à F004 / dashboard MSS — accès patient + signalement priorité au médecin) |
| done-task-028 | **Workflow d'acquittement bio anormale (médico-légal — driver §6.13 / RG-E009-051-052 sécurité)**. Nouveau contrat append-only : `BiologyAck` (table Postgres avec FK `Users` + FK `MailMedicalDocuments ON DELETE CASCADE`, indexes `(DocumentId, CreatedAt DESC)` + `UserId`, PK Guid v7) capture chaque action du médecin sur un CDA portant au moins une valeur biologique `IsFlagged` (codes HL7 `L/H/A/LL/HH/AA`). **5 actions** exposées via `BiologyAckActionType` (publiée dans `dtos-mss` **283.0.0**) : `Acknowledged`, `PatientCalled`, `PatientSummoned`, `ReferredToColleague` (intermédiaires) + `MarkResolved` (clôture explicite). **État de résolution dérivé** (`BiologyAckResolutionState`) calculé à la lecture : `Pending` / `InProgress` / `Resolved`, en mode strictement append (le ré-acquittement après `MarkResolved` re-bascule à `InProgress`). **Endpoints** : `POST /api/v1/medical-documents/{id}/biology-ack` (200/400/403/404) + `GET /api/v1/biology-acks/pending-summary`. **Rôle `Doctor` enforced au niveau service** via `ClaimsPrincipal.IsInRole` (testable en unit test, pas seulement en middleware) ; secrétaire / staff non-médecin → 403. **Audit médico-légal** : 5 nouveaux `AuditActionType` (`BiologyAcknowledged`, `BiologyPatientCalled`, `BiologyPatientSummoned`, `BiologyReferredToColleague`, `BiologyMarkedResolved`) appendés au bout de l'enum (contrat d'ordinal préservé), trace émise post-save avec `DocumentId`. **Note libre optionnelle** stockée sur l'entité `BiologyAck` (`VARCHAR(500) NULL`), validée serveur-side. **Mode offline** : 5 nouveaux `PendingActionTypes` string-constants + `ReplayQueuedAckAsync` qui bypasse le role-check (l'autorisation a déjà eu lieu au click) ; payload JSON déserialisé en `record` positional (Sonar-clean). **Cross-tenant** : isolement via `BaseRepository.GetCurrentUserIdAsync` (silent 404 leak, convention task-023). **Aggregate enrichment** sur `MailRepository.GetMailAsync` (single-mail viewer path) : `MailDto.HasAbnormalBiology` + `PendingBiologyAcksCount` + `HasCriticalPendingBiologyAck` + `MailMedicalDocumentDto.LastBiologyAck` + `BiologyAckState` peuplés via une seule query groupée sur `BiologyAcks` (pas de N+1). **Frontend Blazor v1** : `BiologyAckPanelComponent.razor` (5 boutons + dernière action + critical/warning styling + optimistic update + toast) + `IBiologyAckService` Blazor + 15 clés Localizer FR/EN. **Frontend Angular v1 (code-only)** : `biology-ack.model.ts` + `MssApiService.recordBiologyAck` + `getBiologyAckPendingSummary` + 3 Vitest tests. **Tests** : api-mail **+14 unit `BiologyAckServiceTests`** (5 happy-path Theory + role gate + missing CDA + non-flagged CDA + note length + replay path + replay-not-found) ; Blazor **+7 bUnit `BiologyAckPanelComponentTests`** (render gate + critical/warning class + last-action surface + click → service call + MarkResolved → Resolved) ; Angular **+3 Vitest** sur `mss-api.service.spec.ts`. Suite api-mail **1989/1989** (était 1972 baseline post-task-035 = +17 tests utiles dont 14 BiologyAck + 3 reorgs) ; Blazor **57/57** (+7). **Sonar Phase 1 best-effort** : 154 → ~138 new_code_smells (-16, ~10.4 %), 100 % des findings dans les fichiers task-028 corrigés (S6934 route refactor, S6667 caught exceptions, S1192 const table name, S3459+S1144 positional record, ASP0018) ; 3 CA1873 informational restent sur les loggers (même pattern que 51 findings legacy ailleurs — accepté). Project-wide : `code_smells` 783 → 768 (-15), ratings A/A/A préservés. **PRs** : `dtos-mss#22`, `api-mail#55`, `client-blazor#51` label `awaiting-us-completion` (rule 11). **Déferrals documentés** : confirmation modal pour valeurs `IsCritical`, tuile KPI dashboard, chip filtre inbox « Bio à acquitter (X) », composants Angular UI complets (panel/dialog/tile/chip), aggregate population sur listing paths (`GetMailsByUidsAsync` / `GetMailsByTagAsync`), query param `?onlyPendingBiologyAck=true`, tests d'intégration Postgres-backed pour `BiologyAckRepository` (CASCADE delete, state transitions). | — (pas de `**Closes RG**` déclaré ; pas d'exigence Ségur sur l'acquittement biologique — driver médico-légal, complète §6.13 sécurité / RG-E009-051-052 détection-affichage sans les remplacer) |
| done-task-037 | **MSSanté account onboarding — opt-in flow quand le claim `mssEmail` est absent du JWT (task-037)**. Remplace la cascade silencieuse de 403 (env weda2-archi dev.k8s.office.weda.fr) par un parcours d'opt-in explicite à parité sur les deux frontends. Quand `UserSessionService.HasMssEmail == false` (Blazor) ou que le signal `MSS_USER_EMAIL` retourne vide (Angular), le module MSS est verrouillé sur un écran centré `MssUnconfigured` (« Messagerie non configurée, veuillez configurer maintenant ») + CTA vers `/mss/setup`. La page setup affiche un formulaire (email MSSanté requis + RPPS/ADELI affiché mais non envoyé) qui chaîne 3 calls : (A) `POST /api/v1/account/mss-imap-test` côté api-mail pour valider la connexion IMAP MSSanté avec le PSC token courant, (B) `PUT {KeycloakProxyUrl}/admin/mss-profile` pour persister `mssEmail` sur le compte Keycloak, (C) écran « Déconnectez-vous puis reconnectez-vous » + bouton logout — pas de silent refresh, le humain valide explicitement. **api-mail** : nouveau `AllowMissingMssEmailAttribute` (marker) ; `UserContextEnricherMiddleware` honore la marker et bypass le strict cross-check `mssEmail` claim ↔ `Client-Email` header pour les endpoints onboarding (JWT bearer toujours requis) ; refactorisation `InvokeAsync` → 8 helpers privés pour respecter S3776/S138/S134 thresholds ; `IMssAccountOnboardingService` + impl wrappent IMAP via `IImapClientWrapperFactory` (4 codes d'erreur : `AUTH_FAILED` / `HOST_UNREACHABLE` / `MAILBOX_NOT_FOUND` / `INVALID_EMAIL`) ; nouveau `AccountController.POST mss-imap-test` ; **NuGet `HealthPlatform.Dtos.Mss 289.0.0`** publié (`MssImapTestRequest`, `MssImapTestResponse`, `MssImapTestErrorResponse` + `MssImapTestErrorCodes`). **client-blazor** : `MssUnconfigured.razor` (lock view), `MailSetup.razor` (`@page "/Mail/setup"` — EditForm + DataAnnotations email RFC + RPPS regex 9/11 digits), `MssOnboardingService` (HttpClient direct + JsonContent), `Mss403Handler` DelegatingHandler defense-in-depth sur 403 mss-api, `MssOnboardingGuard` static helper consommé par `Mail.razor` (additif `@if (RequiresOnboarding) { <MssUnconfigured /> }`), `IErrorNotificationService` gain `NotifyMssConfigurationRequired`, Localizer fr/en : 19 nouvelles clés `MssOnboarding_*`. **client-angular (code-only)** : `mssOnboardingGuard` (functional `CanActivateFn` appliqué sur `MSS_ROUTES.canActivateChild`, redirige sur `/mss/unconfigured` quand `MSS_USER_EMAIL()` est vide), nouveau `MSS_KEYCLOAK_PROXY_URL` token (provider sur `environment.authEndpoint`), `MssOnboardingService` (HttpClient + signal-based, retourne typed `ImapTestOutcome` / `PersistOutcome`), `MssSetupComponent` (ReactiveForms + RPPS regex + 3-phase signal `editing/submitting/success`), `MssUnconfiguredComponent` (page lock + CTA), `mss403HandlerInterceptor` (registered dans `app.config.ts` `withInterceptors`), 5 nouvelles routes/composants exposés via `core/index.ts`. **Tests** : api-mail **+18 unit tests** (AccountController 6 + MssAccountOnboardingService 8 + UserContextEnricherMiddleware 4) — suite globale 1891 verts ; client-blazor **+20 tests** (MssOnboardingGuard 4 + MssOnboardingService 7 + Mss403Handler 3 + MssUnconfigured 2 + MailSetup 4) — suite 86 verts ; client-angular **+20 vitest** (guard 5 + service 7 + mss-unconfigured 2 + mss-setup 5 + mss-403-handler 4) — mss-lib 182/182 + weda2 2665/2665. **Sonar Phase 1 (new-code)** : 0 findings restants sur les fichiers task-037 après refactor middleware en 8 helpers et conversion 3 LogXxx call sites en `LoggerMessage` source-generator (CA1873 résolu). Phase 2 (legacy debt) skippée — best-effort par playbook. **Lint Angular** : auto-fix iter 1 → 0 errors, 25 warnings résiduels (legacy `max-lines` hors scope `tag:scope:mss`). **PRs** : `dtos-mss#23`, `api-mail#57`, `client-blazor#52` label `awaiting-human-merge`. **Code-only Angular** : 18 fichiers uncommitted sur `feature/nova-rewriting-mss-develop-20260514` à committer + push TFS + PR par le humain. **Suggestions non-bloquantes** : (a) `MailSetup.OnLogoutNow` (Blazor) appelle `Session.Clear()` + `NavigateTo("/", forceLoad: true)` plutôt qu'un end-session OIDC dédié ; (b) `[ExcludeFromCodeCoverage]` sur le middleware pourrait être revu post-refacto ; (c) la regex RPPS (9 ou 11 chiffres) pourrait être extraite en validator partagé. | — (pas de `**Closes RG**:` déclaré ; contribue à la dimension sécurité / UX du module — cas observé en preprod weda2-archi avec compte Keycloak sans mapper MSSanté ; backend Keycloak proxy `PUT /admin/mss-profile` reste hors scope, spec fournie par l'équipe BFF) |
| done-task-038-fix-mss-imap-test-tls | **Fix TLS callback manquant sur la sonde `POST /api/v1/account/mss-imap-test` (task-037)**. La sonde IMAP d'opt-in onboarding (livrée par task-037) appelait `imapClient.ConnectAsync(...)` **sans installer** de `ServerCertificateValidationCallback` — en déploiement Linux, MailKit retombait sur la validation OS qui rejette la chaîne IGC-Santé → `SslHandshakeException: unable to get local issuer certificate`. **Refactor** : extraction d'un helper `ImapClientTlsConfigurer` (sealed, scoped, exposé via `IImapClientTlsConfigurer`) qui centralise `SslProtocols = Tls12` + `ServerCertificateValidationCallback` avec la logique d'origine (`SslTlsOptions.ValidateCertificate` / `AllowUntrustedCertificates`, `ServerConfig.ValidateServerCertificate`, délégation à `CertificateValidator.ValidateAsync` pour la validation IGC Santé + OCSP/CRL). Les deux services (`MssAccountOnboardingService` ET `ImapConnectionService`) consomment maintenant le helper ; la méthode privée `ConfigureImapClientWrapper` est supprimée de `ImapConnectionService`. `MssAccountOnboardingService` appelle `Configure(...)` **AVANT** `ConnectAsync` (vérifié par test `Received.InOrder`). `CertificateValidator.ValidateAsync` marqué `virtual` (1 mot, OCSP/CRL inchangé) pour permettre le mocking NSubstitute. **Tests** : `+10` (8 helper tests sur toutes les branches du callback : null cert, validation disabled, allow-untrusted, no errors, validator success/failure, Tls12 protocol, callback installation ; +1 ordering `Received.InOrder` ; +1 catch générique pour pousser coverage MssAccountOnboardingService de 94.1% à 98.5%). Suite api-mail : **2032 verts** sur le scope task-038 (flake pré-existant `MarkdownPdfRendererTests.RenderHeadingPreservesText` sur `/F5` UglyToad.PdfPig — non déterministe, passe en isolation, existant sur `develop`, signalé pour task dédiée). **Sonar Phase 1 (new code period scope task-038)** : 0 new bugs / 0 new vulnerabilities / 0 new code smells / 0 new security hotspots sur les 5 fichiers touchés ; new_coverage `ImapClientTlsConfigurer.cs` 95.7%, `MssAccountOnboardingService.cs` 98.5%, `ImapConnectionService.cs` 100% sur lignes nouvelles. Phase 2 (legacy debt) skippée — les 182 new_violations restantes du QG global sont héritées des tasks 018-037 (MailRepository 52, PatientRepository 33, ImapService 7, etc.), pas du scope task-038 ; cleanup legacy = tasks dédiées (`/sonar-s3776` ou chore Sonar ciblées). **Scope OUT respecté** : pas de touch Dockerfile (le code applicatif fait déjà la validation custom), pas de modification OCSP/CRL dans `CertificateValidator`, pas de refactor du `Task.Run(...).GetAwaiter().GetResult()` (anti-pattern reconnu, déféré). **PR** : `api-mail#60` label `awaiting-human-merge`. **Validation HAG en déploiement** : la régression `SslHandshakeException` ne se reproduit pas en local Windows (trust store OS ≠ image Linux) — le humain valide sur env dev k8s avant merge. | — (pas de `**Closes RG**` déclaré ; correctif de bug sur la sonde opt-in task-037 — règle un blocker production sur la fonctionnalité onboarding MSSanté) |
...
- Tasks `archived-task-001.md`, `archived-task-002.md`, `archived-task-003.md`, `archived-task-004.md`, `archived-task-005.md`, `archived-task-006.md`, `archived-task-008.md`, `archived-task-009.md`, `archived-task-010.md`, `archived-task-011.md`, `archived-task-012.md`, `archived-task-013.md`, `archived-task-014.md`, `archived-task-016.md`, `archived-task-017.md`, `archived-task-018.md`, `archived-task-019.md`, `archived-task-020.md`, `archived-task-021.md`, `archived-task-022.md`, `archived-task-023.md`, `archived-task-024.md`, `done-task-026.md`, `done-task-027.md`, `done-task-029.md`, `done-task-031.md`, `done-task-032.md`, `done-task-032bis-fhir-mock.md`, `archived-task-032quater-cda-samples.md`, `done-task-034.md`, `done-task-015a-suppression-detection-backend.md`, `done-task-015b-suppression-decision-endpoint-ui.md`, `done-task-015c-version-navigation.md`, `done-task-036-fix-version-navigation-routing.md` — apports incrémentaux à l'EPIC (cf. Annexe C). Sub-tasks en attente issues des découpages Option B : `todo-task-032ter-greenmail-fixture.md` (Chantier 1 task-032bis, mode `no-code`).
- Tasks `archived-task-001.md`, `archived-task-002.md`, `archived-task-003.md`, `archived-task-004.md`, `archived-task-005.md`, `archived-task-006.md`, `archived-task-008.md`, `archived-task-009.md`, `archived-task-010.md`, `archived-task-011.md`, `archived-task-012.md`, `archived-task-013.md`, `archived-task-014.md`, `archived-task-016.md`, `archived-task-017.md`, `archived-task-018.md`, `archived-task-019.md`, `archived-task-020.md`, `archived-task-021.md`, `archived-task-022.md`, `archived-task-023.md`, `archived-task-024.md`, `done-task-026.md`, `done-task-027.md`, `done-task-029.md`, `done-task-031.md`, `done-task-032.md`, `done-task-032bis-fhir-mock.md`, `done-task-034.md` — apports incrémentaux à l'EPIC (cf. Annexe C). Sub-tasks en attente issues du découpage Option B 2026-05-07 : `todo-task-032ter-greenmail-fixture.md` (Chantier 1, mode `no-code`), `todo-task-032quater-cda-samples.md` (Chantier 3, gated métier).
- Tasks `archived-task-001.md`, `archived-task-002.md`, `archived-task-003.md`, `archived-task-004.md`, `archived-task-005.md`, `archived-task-006.md`, `archived-task-008.md`, `archived-task-009.md`, `archived-task-010.md`, `archived-task-011.md`, `archived-task-012.md`, `archived-task-013.md`, `archived-task-014.md`, `archived-task-016.md`, `archived-task-017.md`, `archived-task-018.md`, `archived-task-019.md`, `archived-task-020.md`, `archived-task-021.md`, `archived-task-022.md`, `archived-task-023.md`, `archived-task-024.md`, `done-task-026.md`, `done-task-027.md`, `done-task-029.md`, `done-task-031.md`, `done-task-032.md`, `done-task-032bis-fhir-mock.md`, `done-task-032ter-greenmail-fixture.md`, `done-task-032quater-cda-samples.md`, `done-task-033.md` — apports incrémentaux à l'EPIC (cf. Annexe C). Les 3 sub-tasks du découpage Option B 2026-05-07 sont désormais toutes livrées. Task-033 (cleanup Sonar massif) achève le chantier qualité.

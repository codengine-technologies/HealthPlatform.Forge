...
> **Dernière mise à jour** : 2026-09-02 (task-288)
...
- **v1.63 — Se déconnecter ferme vraiment la messagerie** (task-285) : jusqu'ici, quitter l'application ne coupait pas la liaison avec l'opérateur MSSanté. Le praticien se croyait sorti, mais une connexion authentifiée **à son nom** restait ouverte plusieurs minutes, le temps qu'un délai d'inactivité l'éteigne — et sur un serveur qui répartit la charge entre plusieurs machines, l'ordre de fermeture n'en atteignait qu'une seule. Désormais, la déconnexion **prévient la messagerie avant de rendre la main**, et cet ordre atteint **toutes** les machines : les connexions tombent en quelques secondes, partout. La déconnexion reste garantie même si la messagerie ne répond pas — le praticien n'est jamais retenu sur un écran de déconnexion, et la fermeture est alors reprise par le mécanisme d'expiration existant. Se déconnecter d'un appareil **ne déconnecte pas les autres** : une session ouverte sur le mobile survit à une déconnexion depuis le poste, et réciproquement.

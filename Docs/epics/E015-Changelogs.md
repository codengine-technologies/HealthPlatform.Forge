...
- **Quatre défauts empilés** :
  ...
  4. **`CanAccessImap => IsOnlineMode`** — un drapeau de **mode**, pas un **état**. `connection/status` a répondu `canAccessImap: true` pendant toute la panne.
...
- **⚠️ CORRECTION DE CONCEPTION EN COURS D'IMPLÉMENTATION** : la première version de la sonde testait `ActiveSessionCount > 0 && ConnectedSessionCount == 0` et aurait déclaré la messagerie morte **au démarrage** — une session qui n'a pas encore ouvert de voie IMAP est légitimement active et non connectée. **C'est le test d'invariant du balayage qui l'a révélé, en contredisant son voisin.** D'où `DisconnectedSessionCount`, qui ne compte que les liens réellement tombés, et sur lequel `CanAccessImap` s'appuie désormais.

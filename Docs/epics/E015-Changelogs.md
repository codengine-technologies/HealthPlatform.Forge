...
### v1.65 — ⚠️ Livraison jumelle de la v1.64 : la fusion du chemin froid ré-implémentée indépendamment, plus la pollinisation LIST-STATUS — task-273

- **Task** : task-273 — `done`. **PR** : `api-mail` [#206](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/206) (label `awaiting-human-merge`). Commits `18a857c` (fusion + tests), `c0a2f7f` (`/forge-simplify`), `759abac` (CA1861 new-code). `dtos-mss` : branche créée, **aucun commit**.
- **⚠️ RECOUVREMENT AVEC task-270 (PR #205), DÉCOUVERT APRÈS OUVERTURE DE LA PR.** Les deux tasks ont fusionné indépendamment le chemin froid de `GET /folders/{name}` (7 → 5 allers-retours, verrou de session unique), dans les **mêmes fichiers** (`ImapService.cs` + les deux mêmes fichiers de tests) — la branche de 273 est partie de `develop`, qui ne contient pas #205. **Les deux PRs ne doivent pas être mergées en l'état** ; l'arbitrage est posé dans `questions/task-273.md` (recommandation : merger #205 d'abord, retravailler #206 pour ne garder que ses apports propres). Défaut de processus consigné : `/develop` n'a pas balayé les `done-*`/PRs ouvertes sur la même zone avant d'implémenter.
...
- **Apports PROPRES à task-273 (absents de #205)** :
  1. **Pollinisation LIST-STATUS** : `GetFoldersAsync` écrit `folder:status:{email}:{path}` pour **chaque** dossier depuis le LIST-STATUS déjà payé (hors verrou, TTL 10 s inchangé, best-effort, dossiers aux compteurs non peuplés sautés) — l'appel `folders` du quatuor arme la validation de `folder/{name}` et `today` quel que soit l'ordre d'émission des fronts.
  2. **Tests d'intégration DOD** (`DashboardArrivalCostIntegrationTests`, vrai `MailClientSessionManager` + vrai enregistreur + cache dictionnaire) : deux arrivées successives → la seconde à **zéro sollicitation IMAP et zéro connexion** ; « pas de cache menteur » — nouveau message servi à l'expiration de la fenêtre de statut ; pollinisation vérifiée.
  3. **Famille de verrou dédiée `GetFolderRead`** (activité + solicitations + verrou) — la table « verrou par opération » de la prochaine campagne isole le chemin fusionné (dans #205, les étiquettes `GetFolderStatus`/`GetFolderQuery` sont conservées).
- **Différence de périmètre vs #205** : task-273 ne porte PAS le remède `today` de 270 (re-validation à 2 commandes au lieu d'une re-recherche à 5) — son chemin `today` reste celui de task-229.
- **Aucun nouveau cache, aucun TTL modifié** ; contrat d'API strictement inchangé (aucun DTO, aucune route).
- **Tests** : build 0 erreur ; `dotnet test` solution **3 898 verts / 16 skips préexistants / 0 rouge** au run de validation. Familles flaky pré-existantes observées sur certains runs solution parallèles (`EnrichmentOperationScopeTests`, `MailRepositoryEnrichPersistInstrumentationTests`, `SeededThreadsAreCountableTests`) — vertes en isolation, fichiers hors diff.

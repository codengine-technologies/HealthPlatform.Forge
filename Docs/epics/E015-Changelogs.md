...
> **Dernière mise à jour** : 2026-09-17 (v1.76)
...
### v1.65 — Le filet d'intégration de l'arrivée dashboard, et une livraison en doublon retirée sans être mergée — task-273

- **Task** : task-273 — `done`. **PR** : `api-mail` [#206](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/206) (label `awaiting-human-merge`), **périmètre réduit le 2026-08-29** : merge `802586f` (deux parents) + réécriture du titre et du corps. Commits d'origine `18a857c` / `c0a2f7f` / `759abac` conservés dans l'historique de la branche mais **neutralisés par la résolution**. `dtos-mss` : branche créée, **aucun commit**.
- **⚠️ RECOUVREMENT AVEC task-270 (PR #205), DÉCOUVERT APRÈS OUVERTURE DE LA PR — ET RÉSOLU.** Les deux tasks avaient fusionné indépendamment le chemin froid de `GET /folders/{name}` (7 → 5 allers-retours, verrou unique), dans les **mêmes fichiers** : la branche de 273 était partie d'un `develop` antérieur à #205. #205 mergée à 16:57 ⇒ #206 `CONFLICTING`. **Arbitrage : ni rebase ni redéveloppement** — le rebase est interdit par la règle 4 et aurait fait résoudre trois fois le même conflit pour jeter ~90 % des hunks ; redévelopper aurait re-dérivé du code déjà mergé. `develop` a été **fusionné** dans la branche avec résolution **intégrale en sa faveur** sur `ImapService.cs` et les deux fichiers de tests unitaires. **Diff final vs `develop` : un unique fichier de tests d'intégration (+227 lignes, zéro code de production).**
- **Défaut de processus consigné** : `/develop` n'a pas balayé les `done-*` ni les PRs ouvertes touchant la même zone avant d'implémenter. C'est la cause racine du doublon, et elle est indépendante de la qualité des deux implémentations.
- **CE QUE LA BRANCHE CONSERVE** — `DashboardArrivalCostIntegrationTests`, le filet de bout en bout que task-270 n'avait pas (ses preuves sont unitaires) : vrai `MailClientSessionManager`, vrai enregistreur de sollicitations, vrai cache dictionnaire, seul le transport IMAP simulé. (1) L'appel `folder` froid paie `resolve` et `STATUS` **une fois chacun** — la propriété livrée par #205, vérifiée sur le geste complet ; une **seconde arrivée dashboard** dans la fenêtre de cache ne paie **aucune** sollicitation ni connexion et sert le même corps. (2) Contrepartie « pas de cache menteur » : un message arrivé entre deux passages est servi dès l'expiration de la fenêtre de statut. **Éprouvés par mutation** : cache toujours en défaut de lecture ⇒ le test de la 2ᵉ arrivée échoue.
- **CE QUE LA BRANCHE ABANDONNE, ET POURQUOI** : (a) la famille de verrou `GetFolderRead` — le schéma de `develop` (`GetFolderStatus` = plancher, `GetFolderQuery` = recherche, dans une seule acquisition) est plus fin et **préserve la comparabilité des séries de campagnes** ; (b) la **pollinisation LIST-STATUS** → `folder:status`, écartée sur un **fait de banc et non une préférence** : le quatuor du dashboard émet `folder` **AVANT** `folders` (`tests/loadtest-k6/scenarios/journey.js`), donc l'amorçage arriverait **après** l'appel qu'il doit servir, et la fenêtre de statut (10 s) est expirée à l'arrivée suivante (~56 s) — **aucun gain mesurable par le harnais actuel**. Bénéfice résiduel réel mais étroit (front affichant l'arborescence avant d'ouvrir un dossier, et seulement sur listing froid : 1 arrivée sur 5, TTL métadonnées 5 min) → à rouvrir en task dédiée, **avec une mesure d'abord**.
- **CE QUE LA TASK GARDE DE PROPRE, MÊME AMPUTÉE DE SON REMÈDE** : le dossier de cause ci-dessous. C'est lui qui a décomposé l'arrivée dashboard phase par phase et **réconcilié deux lectures contradictoires** de l'EPIC ; il reste attribuable à task-273 et vaut indépendamment de qui a livré la fusion.
- **CAUSE ÉTABLIE (exigence DOD, consignée dans la task avant remède)** — décomposition par phase du coût de l'appel `folder` à l'arrivée dashboard (tir 1000 du 2026-08-25) : **verrou** — 2 acquisitions successives (`GetFolderStatus` puis `GetFolderQuery`, attentes p95 5 ms / 450 ms) avec **fenêtre de préemption** entre les deux (l'enrichissement détient le même verrou 6,29 s p95, 34,2 acq/s) ; **IMAP** — 7 RTT dont resolve+STATUS ×2, détention `GetFolderStatus` p95 3,2 s pour 2 RTT (dominée par le re-établissement connect+TLS+auth sous le verrou) ; **base** — aucune requête SQL en régime nominal (id praticien et réglages cachés Redis depuis task-229/074) ; **matérialisation** — négligeable. Réconciliation des « deux lectures » : même route p50 133 ms (dashboard, `folder:status` TTL 10 s toujours expiré après l'inactivité) vs p50 7 ms (inbox 3-10 s plus tard, cache-hit) — deux populations, pas deux coûts.
- **Tests** : build 0 erreur ; `dotnet test` solution **3 897 verts / 16 skips préexistants / 0 rouge** en isolation après réduction. Flaky pré-existants observés sur certains runs solution parallèles (`EnrichmentOperationScopeTests`, `MailRepositoryEnrichPersistInstrumentationTests`, `SeededThreadsAreCountableTests`) — verts en isolation et **structurellement hors de cause** ici, le diff vs `develop` étant un unique fichier de tests d'intégration.
- **Sonar** (analyse complète Release + coverage, serveur relancé par la forge) : bugs 0, vulnérabilités 0, smells 64 → 63, coverage 88,0 → 88,2 %, new coverage 90,9 %, ratings A/A/A, **Quality Gate OK**. 36 issues new-code dont **1 attribuable à la task** (CA1861, corrigée `759abac` ; récidive comptée dans `conventions/csharp.md`, Occurrences : 2).

---

### v1.66 — L'instrument publiait une pointe comme une valeur d'exploitation, et une US allait s'écrire dessus — task-276

- **Task** : task-276 — `done`. **PR** : `api-mail` [#207](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/207) (label `awaiting-human-merge`, `MERGEABLE`). `dtos-mss` : branche créée, **aucun commit**. Commits `989ae12` (instrument), `a436a3e` (passe qualité), `46dc442` (correctif issu de la code review).
- **CE QUI A DÉCLENCHÉ LA TASK.** Le tir discriminant de task-270 (2026-08-29, `journey-500-task270`) a conclu que la fusion du chemin de lecture de dossier avait **allongé la section critique** du verrou `imap_session`, sur la foi d'une détention de **11,871 s** au p95 pour `ReadFolder`, et d'attentes de voisins multipliées par 19. **Cette conclusion était fausse, et c'est la table du rapport qui l'a produite.**
- **DEUX DÉFAUTS EMPILÉS DANS LA TABLE DES VERROUS, PAS UN.** (1) **Fenêtre** — elle couvrait tout le tir, chauffe comprise, alors que task-264 sépare chauffe et régime partout ailleurs dans le rapport et que task-271 avait déjà montré qu'une détention élevée vit souvent dans l'établissement de sessions au front d'un palier. (2) **Réducteur, et c'est le plus grave** — chaque série était réduite à son `max`, c'est-à-dire le **pire instant du tir**, imprimé sans distinction d'une valeur d'exploitation : `11,871 s` et `11,871 s sur 2,8 % des relevés` s'imprimaient à l'identique. Le second défaut n'était pas dans le périmètre initial de l'US : il a été trouvé **en exécutant** l'étape « établir sur pièce », et c'est lui qui portait la fausse conclusion.
- **LIVRÉ** — `regime_window(ctx)` (fenêtre de verdict en datetimes interrogeables, dernier palier = celui qui porte le verdict de capacité, rendu **nommé** dans le titre de la table pour qu'on sache ce qu'elle couvre) ; `collect_prometheus_regime()` (second passage borné, volontairement limité aux séries `imap_session` — élargir doublerait le temps de collecte d'un rapport pour des sections qui ne servent pas à décider d'un correctif) ; `_series_distribution()` (médiane / p90 / pointe / part au-dessus de 2 s) ; une phrase d'attribution qui **suit la mesure dans les deux sens** — elle nomme une pointe non représentative **et** refuse d'excuser une détention soutenue.
- **PIÈGE TRAITÉ AU PASSAGE, ET IL AURAIT SUFFI À VIDER LE REPLIEMENT DE SON SENS** : les requêtes sont des `rate(...[1m])`, donc le premier point d'une sous-fenêtre agrège la minute qui **précède**. Sans décalage, le repliement laissait entrer jusqu'à 60 s de chauffe — en petite quantité, donc **sans se voir**. `PROM_RATE_LOOKBACK_S` décale le début de fenêtre : 60 s de régime perdus sur 2 001 (3 %), contre une contamination silencieuse.
- **LE RÉSULTAT, SUR DONNÉES RÉELLES** (rapport du tir régénéré, fenêtre de régime, palier 500) : `GetFolders` médiane 0,242 s · **`ReadFolder` médiane 0,469 s**, p90 0,560, pointe 11,871, **2,8 %** des relevés > 2 s · `UpdateFlag` 0,496 s (3,1 %) · `GetEmailContent` 0,500 s (5,4 %) · **`GetAttachmentStream` médiane 0,697 s, p90 2,136, 11,6 %**.
- **⭐ LA CONCLUSION DU TIR EST RENVERSÉE.** La détention **typique** de `ReadFolder` (0,469 s) est **INFÉRIEURE** aux 0,692 s de `GetFolderQuery` qu'elle remplace : task-270 a légèrement **raccourci** la section critique, pas allongée. Les 11,871 s sont un transitoire, et la même forme frappe `GetEmailContent` et `GetAttachmentStream` — deux opérations que task-270 **n'a jamais touchées**. Les pointes sont un phénomène de congestion du tir, pas une propriété du chemin fusionné. Le rapport du tir a été **corrigé** : il porte désormais la table de ce qui tient (7 → 5 allers-retours, −31 % d'acquisitions, gain nul au médecin) et de ce qui tombe (les deux affirmations sur le verrou).
- **⭐ LE REMÈDE PRÉVU N'A PAS ÉTÉ ÉCRIT, ET C'EST LE RÉSULTAT DE L'US.** Le task file prévoyait explicitement cette issue : « si la détention ne pèse pas sur le chemin du médecin, l'US s'arrête à son instrument ». Relâcher le verrou aurait corrigé une contention **qui n'existe pas** — répétition de task-222 (US annulée sur une cause supposée), en plus insidieux puisque la cause était ici *mesurée* mais *mal réduite*.
- **FINDING NEUF, CORRECTEMENT SCOPÉ** : `GetAttachmentStream` est la **seule** opération dont la détention est tenue en régime (p90 **2,136 s**, 11,6 % des relevés au-dessus de 2 s). Aucune US ouverte — à instruire.
- **TROISIÈME PIÈGE DE LA MÊME FAMILLE DANS CET EPIC** : buckets calibrés en millisecondes (task-211), plafond d'histogramme lu comme une mesure (task-245), saturation lue comme un timeout (task-271). Même forme à chaque fois — **une non-mesure imprimée avec l'apparence d'une mesure**. C'est la quatrième, et la première où l'artefact avait déjà produit une conclusion publiée.
- **Code review** — APPROVED, **1 défaut trouvé par la review elle-même** : une phase `operate` non mesurée rendait une table à en-tête sans ligne, qui se lit « détentions nulles » au lieu de « aucune mesure ». Corrigé (`46dc442`) ; c'est le mode d'erreur gravé par task-214.
- **Tests** : `tests/loadtest-k6/selftest.sh` **333 verts / 0 rouge** (329 avant, 13 tests neufs), dont les deux tests **symétriques** qui empêchent la phrase d'attribution de dériver dans un seul sens, et la garde de lookback. `dotnet build` 0 erreur / 0 avertissement (aucun fichier C# au diff).
- **⚠️ Sonar NON exécuté** — `$SONAR_TOKEN` absent du shell de la forge et `agents/sonar.md` pointe le port 9001 alors que le serveur répond sur **9000**. Aucun KPI publié, **aucun vert revendiqué**. Le diff est Python, et le harnais k6 **est** scanné par Sonar (42 des 64 violations new-code au tir de task-270) : le risque de dette neuve n'est pas nul, il est **non mesuré**. Écart de documentation à redresser hors de cette PR (règle 6).

---

### v1.67 — Une session poolée fermée par le serveur ne remonte plus une erreur au médecin — task-277

- **Task** : task-277 — `done`. **PR** : `api-mail` [#208](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/208) (`awaiting-human-merge`). Commits `1325371` (correctif), `fc9d116` (passe qualité), `3ead998` (correctif issu de la code review). `dtos-mss` : 0 commit.
- **Le défaut, mesuré sur les deux protocoles** : une connexion poolée que le serveur a fermée de son côté était réutilisée telle quelle — `IsConnected` reflète ce que MailKit croit, pas l'état du socket — et l'échec surgissait sur la **première commande**, rendu au praticien en `ProblemDetails` 500. Deux occurrences au tir du 2026-08-29 : SMTP à 20:29 (`SmtpCommandException` « Service shutting down », sur `MAIL FROM`) et IMAP à 21:34 **en fenêtre de régime** (`SocketException 10054` sur `StatusAsync`, `ElapsedMs=19217`). Contexte : 4 727 `SmtpCommandException` pour 142 467 NOOP (3,3 % — plafond du GreenMail du banc), 1 092 `IOException`, 619 `SocketException` : sur tout ce volume, **deux** ont atteint le médecin, mais le taux croît avec la population. **Pas une régression de task-270** — `MapFolderAccessExceptionAsync` lui est antérieur.
- **⭐ LE POINT DUR EST L'IDEMPOTENCE DU REJEU SMTP, ET IL EST TRANCHÉ PAR LE STADE.** Un courrier médical envoyé deux fois est un incident plus grave que le 500 qu'on supprime. Règle retenue : `SmtpCommandException` **avec** `StatusCode == 421` **et** `ErrorCode` dans {SenderNotAccepted, RecipientNotAccepted} — le **type** prouve le stade (antérieur au `DATA`), le **code** prouve la mort du canal ; aucun des deux seul ne suffirait. `MessageNotAccepted` (DATA émis) et `IOException` pendant l'envoi (stade **indécidable**) ne sont **jamais** rejoués : l'indécidable se traite comme un envoi parti. Un refus métier porte un autre code que 421 et n'est donc jamais rejoué — le rejeu ne masque aucune erreur que le praticien devrait voir.
- **Détection par TYPE, jamais par chaîne** (règle 12), dans un type dédié et testable seul (`DeadSessionPolicy`), avec un test qui construit une exception dont le *message* contient les mots du cas rejouable mais dont le *type* ne l'est pas : il échoue si un `Message.Contains(...)` réapparaît.
- **Défaut BLOQUANT trouvé par la code review** : la première version appelait `RemoveSession`, qui **détruisait la session dont la lecture détient le verrou**. La reprise se serait exécutée sur une session neuve, **sans protection**, et un appel concurrent du même praticien aurait pu s'intercaler — l'invariant même que `imap_session` existe pour tenir. Corrigé : on ferme le **client**, la **session** survit, `ConnectInternalAsync` re-établit dessus. Le test assert les **deux moitiés** de l'invariant.
- **Reconnexion SMTP en place** (`ReconnectSessionClientAsync`) : ni nouvel emprunt (interblocage — le jeton détient le verrou SMTP), ni libération/ré-emprunt (fenêtre où un autre envoi prendrait la session). Réutilise `CreateConnectedClientAsync`.
- **Compteur `mssante_mail_session_retry_total{lane,outcome}`** : quand la reprise marche, le 500 disparaît et le défaut devient **invisible** — sans compteur, la disparition du signal serait indiscernable de l'absence du problème (task-214). Journalisation en **Information** : le recyclage de connexions est nominal, mais la trace doit rester pour voir une dérive du serveur distant.
- **Passe qualité** : `InvalidOperationException` (500) → `UnavailableException` (**503**) — un serveur injoignable est *indisponible*. Factorisation des deux blocs de reprise **écartée volontairement** : elle aurait déplacé la garde d'idempotence derrière une abstraction, alors qu'elle doit rester lisible à son site d'appel.
- **Tests** : 13 neufs. Suites unitaires **3 491 vertes** (domain 136, infra 464, api 692, application 2 199). Les 5 rouges d'intégration sont **pré-existants et vérifiés comme tels** — diff mis de côté, mêmes tests rejoués sur `develop` nu, mêmes échecs : flaky « du jour » de minuit (F-259-1), le tir a eu lieu à 00:30 locale.
- ⚠️ **Sonar non exécuté** (`$SONAR_TOKEN` absent, doc port 9001 vs serveur 9000) — diff **C# applicatif**, dette neuve **non mesurée**.

---

### v1.68 — Le téléchargement d'une pièce jointe entre dans le compteur de sollicitations — task-279

- **Task** : task-279 — `done`. **PR** : `api-mail` [#209](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/209) (`awaiting-human-merge`), commit `a572062`. `dtos-mss` : 0 commit.
- **L'angle mort** : au tir du 2026-08-29, `GetAttachmentStream` était **la seule** opération dont la détention du verrou soit *tenue en régime* (médiane 0,699 s, la plus élevée ; 11,6 % des relevés > 2 s) et **la seule** dont les commandes IMAP n'étaient comptées par rien — **0 entrée**. La lecture du code donnait quatre commandes ; rien ne le prouvait, et rien n'aurait mesuré une correction. Même angle mort que task-262 avait fermé pour les dossiers.
- **⭐ Le point qui décide de la valeur du compteur : ne compter que les ouvertures RÉELLES.** MailKit court-circuite `OpenAsync` quand le dossier est déjà ouvert avec le même accès ; compter l'appel plutôt que la commande aurait rendu un **majorant** et fait croire à une redondance inexistante — or c'est **exactement la question** que ce compteur existe pour trancher (le médecin ouvre le message *puis* télécharge sa pièce jointe, dans le même dossier).
- **Aucun remède écrit, volontairement** : la redondance du SELECT est une question de **fréquence**, qui se lit au banc, pas de **mécanisme**. L'US l'autorisait explicitement.
- **Deux causes plausibles déjà écartées par la mesure**, et l'US interdit de les réinstruire : l'écriture base sous le verrou (`assemble` 6,6 ms moyens, 4 % de la détention — hypothèse de lecture de code **réfutée**) et la contention subie (attente médiane **5 ms** : cette opération n'attend pas, elle occupe).
- **Tests** : 5 neufs — les 4 commandes **dans l'ordre** ; pièce jointe introuvable → les 3 déjà envoyées **quand même** comptées ; servi par le cache base → rien ; connexion en échec → rien (absence prouvable) ; dossier déjà ouvert → pas de `open_folder`. Suites unitaires **3 483 vertes**.
- **Piège d'outillage consigné** : `FetchAsync(IList<UniqueId>, MessageSummaryItems, CancellationToken)` est une **méthode d'extension** MailKit — NSubstitute ne peut pas l'intercepter et rend une `RedundantArgumentMatcherException` au symptôme trompeur. Le membre d'interface prend `IFetchRequest`.
- ⚠️ **Sonar non exécuté** (même cause).
...
- **⭐ LA CAUSE : l'appel n'est pas cher, il est FROID — et il l'est par construction.** Le harnais émet la **même route deux fois par itération**, à quelques secondes d'écart, sur la même session : `dashboard,call:folder` **358,4 ms** (p50 140,3) contre `read_list,call:folder` **52,7 ms** (p50 **10,3**) — **×6,8 sur la moyenne, ×13,6 sur la médiane**. Même route, même travail, même dossier ; le seul écart est la **position dans le parcours**. Un passage dure ~63 s : toutes les fenêtres de cache ont expiré quand le médecin revient sur son tableau de bord. Reproduit à l'identique au tir de référence. C'est la réconciliation que le dossier de cause de task-273 avait écrite (« deux populations, pas deux coûts ») — désormais **chiffrée**, et prouvée survivante à task-270.
- **Écart client/serveur : vérifié, il n'y a rien.** Une première lecture donnait 185 ms — **faute de comparaison** (moyenne client d'un seul des deux appels contre moyenne serveur des deux). Recalculé : client 205,5 ms, serveur 172,9 ms → **32,6 ms**, l'ordre de grandeur d'un aller-retour localhost via le proxy DCP.
...
### v1.70 — Le chemin d'archivage comptait zéro aller-retour : l'angle mort qui a tenu trois US en échec — task-281

- **Task** : task-281 — `done`. **PR** : `api-mail` [#213](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/213) (`awaiting-human-merge`), commit `490b032`. `dtos-mss` : 0 commit.
- **Filiation** : c'est le « relevé annexe » de v1.69 qui a ouvert cette US — l'attente moyenne du verrou de session sur `ReadFolder`, 82,8 ms, ~48 % du temps serveur de la route. Le tir du 2026-08-30 l'a attribuée grâce à l'étiquette `holder` : **`ReadFolder` derrière `AppendToSent`, p95 médian 3,938 s**, max 20,357 s. Contre-épreuve décisive : **verrou libre, 0,005 s** — c'est de la **contention pure**, pas un coût d'acquisition, et les deux situations ont des remèdes opposés.
- **⭐ L'ANGLE MORT, ET IL EST STRUCTUREL** : `AppendToSent` était le **seul** chemin IMAP à n'enregistrer **aucune** sollicitation serveur — zéro appel à `mailServerSolicitations.Record` dans tout `TryAppendToSentAsync`. Les chemins de **lecture** les comptent tous depuis task-225 puis task-262 ; les chemins d'**écriture** (`AppendToSent`, `AppendToDrafts`) étaient restés dehors. On savait *combien de temps* le verrou était tenu (task-211/213), *derrière qui* on attendait (`holder`, task-278), et si la détention payait l'établissement (`phase`, task-271) — **jamais de combien d'allers-retours cette détention était faite.** Voilà pourquoi task-276, task-277 et task-278 ont toutes buté dessus sans pouvoir l'attribuer, et pourquoi task-276 a même publié une cause fausse.
- **⭐ LA DÉTENTION N'EST PAS « UN APPEND »** — décomposition par lecture du code, tout entier **sous le verrou** : `ConnectInternalAsync` (**CONNECT + TLS + AUTH** si session froide) → `TryResolveSpecialFolderAsync(Sent)` (**non mémoïsée**, refaite à chaque envoi, et en repli elle **sonde les noms candidats un par un**) → `OpenAsync(ReadWrite)` (SELECT) → `AppendAsync` (transfert du message entier) → `CloseAsync`. **Quatre allers-retours au minimum.** Le nom de l'opération désignait la quatrième ; le verrou couvre les cinq étapes.
- **Livré** : `MailServerCommands.AppendMessage` (`append_message`, la commande manquait au vocabulaire) ; les 4 allers-retours comptés et étiquetés `operation="AppendToSent"` — **la même chaîne que celle passée au verrou**, sans quoi la table `holder` et le décompte de sollicitations ne se recouperaient pas au banc ; `ResolveFolder` compté **avant** le contrôle de validité, convention de task-262 (la résolution coûte un aller-retour même quand elle ne trouve rien).
- **⛔ AUCUN REMÈDE, ET C'EST LE RÉSULTAT** — le DOD l'autorisait explicitement. Le piège task-213/215/216 (seconde session IMAP pour l'archivage, **mesurée puis rejetée** : l'envoi était *plus lent* avec elle, p95 7 874 ms sans contre 10 439 et 12 573 avec) est évité de front : rien n'a changé depuis qui justifierait d'y revenir, et surtout **la mesure qui désignerait un remède n'existait pas encore**. C'est précisément ce que cette livraison rend possible.
- **Piste désignée sans être tranchée** : l'étape de résolution est la seule du lot qui soit du **travail répété sans nécessité** — le dossier « Envoyés » d'un praticien ne change pas d'un envoi à l'autre. Candidat de « réduire la détention » qui ne rouvre pas task-213. **À confirmer par le tir** : si `resolve_folder` pèse peu face à `append_message`, la piste ne vaut pas le changement.
- **Tests** : 3 neufs, cycle **RED → GREEN vérifié** (`Actual: []` avant instrumentation, 3/3 après) — les 4 commandes **dans l'ordre**, l'**attribution** à la famille d'opération, et le chemin d'échec (boîte sans dossier « Envoyés » : **une seule** sollicitation comptée, celle de la résolution — facturer un OPEN ou un APPEND qui n'ont pas eu lieu gonflerait le coût du chemin dans le rapport, l'erreur qui trompe le plus). Assertions sur des **nombres**, des **ordres** et des **étiquettes**, jamais sur des temps. Suite complète **3 932 vertes** en Release.
- **✅ Sonar exécuté** — pour la première fois de la série récente (task-279 et task-278 avaient dû y renoncer, `$SONAR_TOKEN` absent). Serveur relancé, `UP` sur **9001** et non 9000 comme l'annoncent `agents/sonar.md` et le skill. Quality Gate **ERROR** avant comme après, mais **rien de rouge n'est attribuable** : baseline du 2026-08-23, soit une semaine de `develop` (tasks 276→280) dans le delta. Vérifié directement — **0 issue et 0 hotspot sur les trois fichiers touchés** ; sur 15 hotspots `TO_REVIEW`, aucun sur ce diff. Couverture 88,0 → **88,1 %**.
- **⚠️ Limite assumée, et elle porte sur la moitié du DOD** : la décomposition est **structurelle**, prouvée par le code et par les tests ; les **chiffres de répartition** (part de l'établissement, de la résolution, de l'APPEND) **n'existent pas encore** et sont reportés au tir de clôture. Le rapport de référence `report-journey-500-lot277279-20260830-180647.md` est **git-ignoré** (`.gitignore:386`, seul `INDEX.md` est suivi) : c'est un artefact local de la machine qui a rédigé l'US, absent du clone qui l'a implémentée. Les chiffres cités ici viennent du task file, pas d'une lecture directe.
- **Note d'outillage, coûteuse à redécouvrir** : `dotnet test --artifacts-path …` **fait échouer une dizaine de tests qui scannent les sources** (`SecretLiteralScan`, `MailContentWriterScan`, `DataContextGetterScan`, `SslTlsOptionsBinding`, `EmbeddingOptionsConsistency`). `TrackedSourceScan.RepoRoot()` remonte depuis l'assembly pour trouver la racine du dépôt ; avec ce drapeau les binaires en sortent et `RepoRoot()` rend `null`. Le contournement des verrous MSB3021 se paie donc de **~10 faux rouges** — arrêter l'AppHost et utiliser les chemins standards.

> **⚠️ Correction de contexte sur v1.69** : la livraison qu'elle décrit (arrivée dashboard servie sans attendre, *stale-while-revalidate*) a été **retirée par task-280** (`revert(mail)`, commit `041cd91`) — « elle dégradait ». L'entrée v1.69 reste telle quelle (historique append-only), mais elle ne décrit **pas** l'état de `develop`. task-280 n'a pas encore d'entrée propre ici.

---

### v1.71 — Un flag absent éteignait l'étage IA en entier, et deux tours de revue ont trouvé des tests qui ne gardaient rien — task-289

- **Task** : task-289 — `done`. **PR** : `api-mail` [#217](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/217) (`awaiting-human-merge`), branche `fix/task-289-flag-absent-isole-du-snapshot`, 4 commits. `dtos-mss` : 0 commit.
- **Origine** : incident **observé en exécution réelle** en Staging le 2026-09-02 à 17:44, pas déduit d'une lecture de code. Les 8 flags `dashboard_widget_*` de task-274 n'avaient jamais été créés dans l'environnement Flagsmith (leur création est un geste d'exploitation, le seeder ne sert que le banc).
- **⭐ LE DÉFAUT, ET IL EST STRUCTUREL** : `RefreshAsync` évaluait les 11 flags de `FeatureFlags.All` dans **une seule boucle**, et `IsFeatureEnabled` **lève** sur un flag inconnu de l'environnement. La première absence avortait donc le rafraîchissement **entier** : `_snapshot` n'était jamais affecté (l'affectation suit la boucle), les 11 flags retombaient sur leur repli de démarrage à froid — et ce repli n'est pas neutre, les widgets sont fail-open (`true`) mais les `ai_*` sont fail-closed (`false`). Conséquence : **l'étage IA éteint pour tous les praticiens, et un 404 « AI summary is not available » sur chaque demande de résumé d'un mail clinique, à cause d'un flag sans aucun rapport avec lui.** Les valeurs des `ai_*` avaient même été lues *avec succès* — elles sont en tête de `All` — puis jetées avec le reste.
- **⚠️ Le message d'erreur mentait** : « serving last known flag state » était écrit alors qu'**aucun état n'avait jamais été connu**. C'est ce libellé qui a rendu l'incident illisible pendant des heures.
- **⭐ LE PIÈGE D'HÉRITAGE, vérifié par réflexion sur le paquet Flagsmith 9.0.0** : `FlagsmithAPIError` **hérite de** `FlagsmithClientError`. Un `catch (FlagsmithClientError)` nu attrape donc **aussi les pannes de transport** — le premier jet publiait un snapshot de replis à chaque hoquet réseau, ce qui **détruisait l'invariant « dernier état connu prime » de task-199**. Le filtre est `when (error is not FlagsmithAPIError)`, et c'est un **test de non-régression** qui l'a attrapé, pas la relecture. Les messages d'exception de l'assembly ont été énumérés pour établir que le seul `FlagsmithClientError` nu joignable depuis `IsFeatureEnabled` est bien « ce flag n'existe pas ».
- **Livré** : `EvaluateDeclaredFlagsAsync` (isolation par flag — le flag absent prend son repli déclaré, les autres portent la valeur servie) ; statut de rafraîchissement **`partial`** distinct de `success` et de `failure`, avec **son propre throttle** (un écart de configuration n'est pas une panne, les confondre rend l'alerte inexploitable) ; log de dérive qui **nomme** les flags absents ; log de panne véridique, qui nomme les flags que le repli laisse **éteints** ; symétrie sur le chemin d'identité praticien (task-274) ; `FeatureFlagWarmUpService`, amorce au démarrage.
- **`await Task.Yield()` en tête de l'amorce, et ce n'est pas décoratif** : `BackgroundService` n'attend `ExecuteAsync` que jusqu'au premier `await` réellement **incomplet**. En évaluation locale Flagsmith tout se complète de façon synchrone, donc sans le yield l'appel réseau serait payé par le **thread de démarrage du pod, sous le verrou du service** — exactement ce que l'amorce prétend éviter. Même motif que `BackgroundTaskQueue`.
- **⭐ DEUX TOURS DE REVUE, TOUS DEUX EN CHANGES REQUESTED, ET LE SECOND A TROUVÉ UN DÉFAUT QUE LE PREMIER AVAIT CAUSÉ.** Tour 1 : deux tests restaient **verts sous mutation** — l'un avec `FailClosedAtColdStart` vidée (donc quand le log de panne cesse de nommer `ai_pipeline`), l'autre **avec le correctif d'isolation annulé** (il assertait les flags à leur valeur de repli, or un rafraîchissement avorté rend justement les replis) — plus un libellé d'amorce qui affirmait « state loaded » sans le vérifier. Tour 2 : la correction de ce libellé par une **heuristique** (comparer chaque flag à son repli) **mentait** — sonde : Flagsmith **répond**, widgets ON et `ai_*` OFF (la configuration même d'avant le déploiement de l'étage IA), et l'amorce annonçait « NO state came back from Flagsmith … ai_pipeline stay DISABLED » **en supprimant la ligne vraie**. Un warning faux à chaque démarrage désensibilise l'exploitation à l'alarme que l'US crée. Et dans le cas où l'heuristique disait vrai, elle **dupliquait mot pour mot** `LogRefreshFailure`. Sa seule contribution propre était son faux positif : **retirée**. Le remède final est l'inverse d'un ajout — ne rien affirmer, n'énoncer que du mesuré.
- **Contre-épreuves finales, chiffrées** : `FailClosedAtColdStart` vidée → **2** tests tombent ; correctif d'isolation neutralisé → **7** ; niveau de log de l'amorce muté → **3** (il laissait 45/45 vertes avant) ; heuristique réintroduite → **2** ; `ai_pipeline` passé en fail-open → **6**.
- **Tests** : 11 unitaires d'isolation (dont la régression nommée de l'incident et la non-régression « panne de transport ≠ flag absent »), 6 sur l'amorce, 2 de convention, 3 d'intégration sur `GET /api/v1/FeatureFlag` (règle 1b). **Test de convention** ajouté : tout flag de `All` doit avoir un repli **explicitement déclaré**, avec ancrage littéral de la liste fail-closed — ajouter un 4e flag fail-closed devient une décision visible au lieu d'un effet de bord. Suite complète **4 026 vertes**.
- **✅ Sonar** : Quality Gate **OK** sur 5/5, **0 issue sur les 15 fichiers du diff**, `new_bugs`/`new_vulnerabilities`/`new_security_hotspots` à 0, `new_coverage` 91,0 %. Fichiers ajoutés à **100 %** de couverture ; les 16 lignes non couvertes de `FlagsmithFeatureFlagService` sont **toutes pré-existantes** (task-199/201/274), inspectées ligne à ligne via `api/sources/lines`.
- **⚠️ Réserve de méthode, à ne pas lire comme un succès** : la cible `new_coverage ≥ 95 %` du playbook **n'est pas opposable** sur ce serveur — `new_lines` dépasse le `ncloc` total du projet, la période « new code » ne délimite donc rien d'utile. La couverture du code neuf a été établie **fichier par fichier**, ce qui l'est. De même, `code_smells` 62 → 66 n'est **pas attribuable** : 0 issue sur les fichiers de la task, et la baseline datait du 2026-09-01, antérieure au merge de task-285 dont la branche part.
- **Boucle d'auto-amélioration** : un seul finding Sonar sur tout le cycle, **CA1859**, et il a été **introduit par la passe qualité `/simplify` elle-même** sur recommandation d'une revue (« le helper ne mute rien » — vrai mais hors sujet sur un helper privé). Récidive d'une règle documentée depuis task-203 → `conventions/csharp.md` incrémenté à 2 occurrences, consigne étendue aux **paramètres** et non plus aux seules valeurs de retour, avec un paragraphe « ne pas confondre avec l'immuabilité ».
- **⚠️ `conventions/` n'était pas versionné** — absent de la liste de ré-inclusion du `.gitignore` (qui ignore tout par défaut), alors que CLAUDE.md en fait un artefact du plan de contrôle. Les leçons apprises ne survivaient donc ni à un clone ni à un changement de poste : la boucle se réinitialisait en silence. Corrigé.
- **Notes d'outillage, coûteuses à redécouvrir** : (1) le pré-flight `/sonar` ne démarrait **que** `sonarqube`, pas sa base — `docker start sonarqube` rend la main **sans erreur**, puis le serveur s'arrête seul sur `UnknownHostException: sonarqube_db` et repasse en `Exited (0)`, code de sortie **0** ; le poll de 90 s expirait alors en concluant à tort « serveur pas prêt », et seul `docker logs` disait la vérité. Corrigé dans `agents/sonar.md`. (2) La version du conteneur a **rebasculé de 25.6.0 à 9.9.8 en deux jours** (donc `sonar.login` et non `sonar.token`) : cinquième bascule du tableau du playbook, et la première *descendante* — il n'y a aucune tendance à extrapoler, le `curl` de contrôle reste le seul geste fiable.
- **⚠️ Flakies pré-existants, attribués et non masqués** : les tests de télémétrie de `mss.mail.infrastructure.tests` et `mss.mail.application.tests` échouent par intermittence sous la charge d'une exécution solution entière, sur un **test différent à chaque run**. Attribution établie par **contre-épreuve sur worktree vierge d'`origin/develop`** : deux exécutions, dont une rouge sur d'autres tests de la même famille, à code identique. Aucun n'est dans le diff de task-289.
- **Suite ouverte** : `todo-task-290.md` — les mails traités pendant la panne ont **sauté l'étage IA** et le resteront après réactivation du flag (pas d'embedding, pas de tags, **pas d'extraction de contacts** — qui alimente le rattachement patient). Le résumé, lui, se régénère à la demande. Le mail sauté n'étant **pas marqué**, aucun critère ne distingue « sauté par flag » de « échoué côté OpenAI » ou « légitimement vide » : le marquage doit précéder le rejeu.
- **Points signalés, non traités (arbitrage)** : log de résolution de dérive absent (`_flagsmithFailures` a son `TryConsumeRecovery`, `_driftReports` n'a aucun pendant) ; throttle de dérive unique pour les deux scopes, donc une dérive d'environnement peut étouffer le premier log d'une dérive d'identité ; tag `scope` sur le compteur (changement du contrat de télémétrie) ; invariant d'ordre `slot.Gate` → `_gate` désormais sur le chemin nominal, sans deadlock (ordre vérifié dans les deux sens) mais écrit nulle part ; health check `feature-flags` en `Degraded`, plus exploitable qu'une ligne de log au boot mais impliquant une décision produit.

---

---

### v1.72 — Le multiplexeur PostgreSQL fait attendre au lieu de rejeter, et le rejet devient une erreur typée — task-294

- **Task** : task-294 — `archived` (mergée le 2026-09-11). **PR** : `api-mail` [#227](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/227), squash `d04f2ca`, branche `feat/task-294-pgbouncer-wait-not-reject`, 3 commits. `dtos-mss` : 0 commit (aucun changement de contrat).
- **Origine** : campagne task-292 du 2026-09-09. À 1000 praticiens, PgBouncer répondait `08P01 connect timeout (server_login_retry)` — **646 refus** au drain sérialisé, **15 873** au drain parallèle — au lieu de faire attendre. `sv_login` mesuré à **212-243** backends simultanément en cours d'ouverture : la signature d'une réouverture permanente des 1000 pools.
- **⭐ UN REFUS N'EST PAS UNE ATTENTE** : un `08P01` est un **500 servi au médecin en ~10 ms**. Il déleste le trafic, donc il **flatte** les latences des demandes servies — un tir qui refuse publie des chiffres optimistes tout en étant en panne. C'est ce qui a fait passer 85 787 refus inaperçus au tir du 08/09, où le rapport a dû être complété à la main.
- **Livré — profil de banc `pgbouncer.ini`** : `server_idle_timeout` 60 → 600 et `server_lifetime` 600 → 3600 (fin de la réouverture permanente), `server_connect_timeout` 15 → 30, `server_login_retry` relâché. Chaque valeur est commentée par la mesure qui la justifie.
- **Livré — rejet typé** : `GlobalExceptionHandler.IsDatabaseUnavailable` mappe `PostgresException` `08P01` / `08006` / `57014` en **503 `ProblemDetails`**, en cherchant le code **n'importe où dans la chaîne d'`InnerException`** (la stratégie d'exécution EF Core enveloppe le driver, donc le code n'est jamais au premier niveau). Aucune fuite de détail technique au client.
- **Non-rejeu prouvé, pas supposé** : le chemin de données (`UseNpgsql` sans `EnableRetryOnFailure`) ne rejoue pas — un test le démontre (délégué tenté une fois, `08P01` en cause racine), au lieu de le déduire de la configuration.
- **Harnais** : `observe.ps1` échantillonne `sv_login` et les refus `server_login_retry` **par delta** (`docker logs --since` le relevé précédent, jamais un cumul depuis le démarrage du conteneur — qui aurait fait hériter chaque palier des refus du précédent et coûté O(taille du journal) toutes les 5 s). `report.py` publie `sv_login` (moy/max) et le **total de refus du palier** dans « Coûts résidents contre N » ; **tout refus non nul classe le tir ROUGE**.
- **Passe qualité** : 10 fichiers — mesure des refus par delta plutôt qu'en max cumulé, filtrage de motif sans allocation.
- **⚠️ Trois items de DOD différés au banc (HAG)** : A/B sur au moins deux jambes du protocole 1000 r2, re-tir 1000 final (erreurs ≤ 0,1 %, `08P01` = 0), et la réécriture « par la mesure » des commentaires de l'ini qui en découle. 3 h 30 par tir sur le banc distant — hors portée d'un cycle de forge.

---

### v1.73 — Le banc lit le coût du login PostgreSQL et le churn des backends, au lieu de les déduire — task-295

- **Task** : task-295 — `done`. **PR** : `api-mail` [#228](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/228) (`awaiting-human-merge`), branche `feat/task-295-bench-postgres-login-probe`, 4 commits. `dtos-mss` : 0 commit (branche créée par auto-inclusion, aucun changement de contrat).
- **Origine** : `Docs/plan_remediation_fable.md` phase 0, sur les tirs des 08 et 09/09. Le diagnostic du `08P01` de task-294 tenait sur **trois grandeurs relevées à la main pendant le tir** : un `psql` chronométré dans le conteneur (**10 à 16 s**, contre 6 ms sain), `sv_login` à 212-243, le cgroup mémoire à 99,9 %. Aucune n'était dans le harnais — le verdict rouge restait une **déduction reconstituée après coup**, jamais une lecture sur une même série temporelle.
- **⭐ CE QUI GOUVERNE L'INCIDENT EST LE DÉBIT DE CRÉATION DE CONNEXIONS, PAS LE DÉBIT DE REQUÊTES.** Le harnais publie désormais `started_last_60s` (backends nés depuis moins de 60 s) à côté des req/s, et `idle_over_60s` — dont un **zéro soutenu** signifie que *tous* les backends inactifs sont recyclés (churn total), ce que `server_idle_timeout = 60` produit sur 1000 pools. C'est le témoin direct de la jambe `server_idle_timeout` de task-294.
- **Livré — trois sondes SÉPARÉES dans `observe.ps1`** (l'invariant « une sonde en échec ne coûte jamais la ligne des autres » interdisait de les fusionner) : `Sample-Postgres` étendu (`started_last_60s`, `idle_over_60s`, `pooler` / `direct` par `client_addr`, dans la requête `pg_stat_activity` existante) ; `Sample-PostgresLogin` (`psql` **dans** le conteneur, ni réseau ni pooler — le coût que PgBouncer paie lui-même pour ouvrir un backend ; base praticien fixée au premier relevé et journalisée, pour que la série soit comparable) ; `Sample-PostgresCgroup` (`usage_pct`, `cache_mb`, `rss_mb`, `majfault_per_s` en delta).
- **Livré — `report.py`** : « Coûts résidents contre N » porte login p50/p95/max, le churn et le cgroup **par palier** ; deux avertissements (`⚠️ login PostgreSQL > 1 s`, `⚠️ sv_login > 20 soutenu`) qui ne classent **pas** le tir ROUGE — un login lent sans refus est une alerte, le système tient encore, et seul le refus est disqualifiant depuis task-294 ; et une **ligne de corrélation** à trois branches, reprise dans le motif du verdict rouge : « refus ET login p95 > 1 s → le login lent est la cause probable » / « refus ET login p95 < 1 s → la cause n'est pas le login, lire `SHOW LISTS` et le DNS » / « refus ET login non relevé → corrélation impossible, armer la sonde avant de conclure ».
- **⭐ DEUX DÉFAUTS TROUVÉS *AVANT* LA PR, ET AUCUN N'ÉTAIT VISIBLE À LA RELECTURE** :
  1. **Le chronomètre rendait toujours 0.** Le `date +%N` de l'image **Alpine** (busybox) du Postgres du banc ignore les nanosecondes : un login de 6 ms se lisait « 0 s ». Remplacé par `$EPOCHREALTIME` de bash 5. Trouvé en **exécutant** la sonde contre le conteneur, pas en la relisant — sans cette exécution, la task livrait une colonne de zéros, c'est-à-dire exactement le mode d'échec silencieux qu'elle existe pour empêcher.
  2. **La sonde redéfinissait un compteur historique** (commit `303eb98`, trouvé en revue de code). Le `where backend_type = 'client backend' and pid <> pg_backend_pid()` était **global** : il retranchait aussi les processus d'arrière-plan de `backends/total` et `practitioner_databases`, deux séries comparées d'une campagne à l'autre depuis task-204. Mesuré sur le banc pendant la revue : `total` serait passé de **566 à 560** — six processus perdus en silence, donc toute comparaison avec un tir archivé faussée sans que rien ne le signale. Le filtre vit désormais dans les `filter (where …)` des quatre nouveaux compteurs seuls.
- **Absence ≠ zéro, jusqu'au bout** : sur un hôte sans cgroup v1, la sonde n'émet **aucune** ligne `postgres,cgroup,*` (journalisée une fois) et `report.py` **omet** les lignes cgroup au lieu de les afficher à 0. Une contre-épreuve dédiée le vérifie.
- **Validation sur le banc réel** (pas seulement sur fixtures) : les 11 métriques `postgres/*` sortent ; login mesuré **5,1 à 7,6 ms** contre une référence saine documentée de 6 ms ; cgroup 79,7 → 93,8 % ; `majfault_per_s` bien absent au premier relevé.
- **Tests** : 16 neufs (10 dans `test_report_resources.py`, 6 dans `test_report_conclusions.py`) et 3 fixtures — `observe-login-slow.csv` (reconstruite des mesures manuelles du 09/09), `observe-login-healthy-refusals.csv` (refus **avec** login sain — la contre-épreuve qui empêche de conclure trop vite), `observe-login-sain.csv`. `selftest.sh` : **354 tests verts**.
- **Passe qualité** (`7c0a31e`) : `sustained_share` (task-212) réutilisé au lieu d'être réimplémenté ; spec de lignes générale `(scope, target, metric, label, stat, digits)` rendue par une seule boucle ; libellés de seuil **dérivés** des constantes, pour qu'un seuil qui bouge ne laisse pas un texte qui ment ; `Invoke-ContainerScript` encapsule le piège PowerShell 5.1 des guillemets doubles et **refuse bruyamment** un script qui en contient.
- **✅ Sonar** : Quality Gate **OK**, **0 issue** et **0 hotspot** sur le new code (2 `CA1869` corrigées — `JsonSerializerOptions` construit par appel, hérité de task-292). `new_coverage` 88,0 → **91,5 %**, coverage projet 88,1 → **88,6 %**, bugs 0, vulnérabilités 0, ratings A/A/A. Convention `CA1869` ajoutée à `conventions/csharp.md`.
- **⚠️ Réserve de méthode** : `new_coverage` reste sous la cible de 95 % du playbook. La période « new code » du projet est `PREVIOUS_VERSION` — elle couvre tout le C# mergé par task-292/293/294 le même jour — et **cette task n'ajoute aucune ligne de C#** (le livrable est du Python et du PowerShell). L'écart ne lui appartient pas et n'est pas comblable depuis son périmètre. Dit ici plutôt que tu, conformément au principe de l'EPIC.
- **⚠️ Note d'outillage** : au moment du `/review`, cinq processus `mss.mail.api.exe` du banc verrouillaient `bin/Debug` — la validation a été faite en **Release** plutôt qu'en tuant le banc de l'humain. Le SonarQube du poste avait aussi besoin d'un redémarrage (`sonarqube_db` **puis** `sonarqube`), et il répondait en **9.9.8** sur le port **9000** : `docker port sonarqube` fait foi, pas la dernière ligne de l'encadré du playbook.
- **⛔ Un item de DOD différé au test humain (HAG)** : un tir court (200 praticiens, ≥ 20 min, `journey`) qui montre les 9 colonnes sans trou, produit son rapport `.md` et sa ligne `INDEX.md`. Exige l'AppHost en profil loadtest et un seed — hors portée d'un cycle de forge, c'est le Manual Test Plan de la PR.

---

---

### v1.74 — Le cache partagé cesse de se saturer seul : ce qui est trop gros n'y entre plus — task-297

- **Task** : task-297 — `done`. **PR** : `api-mail` [#229](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/229) (`awaiting-human-merge`), branche `feat/task-297-cache-borne-taille-entree`, 4 commits. `dtos-mss` : 0 commit (branche créée par auto-inclusion, aucun changement de contrat).
- **Origine** : campagne task-292 du 2026-09-09, finding **F-292-3**, tranché par le tir A/B « journal désactivé ». Le finding avait d'abord été attribué au spill d'audit (task-186/292) ; **la mesure l'a exonéré** — 6 568 `Timeout getting key` sur le tir **sans journal**, donc sans aucune autre charge Redis.
- **⭐ LA MÉCANIQUE, ET ELLE EST MONO-THREAD** : Redis exécute **une commande à la fois**. Le slowlog porte des `HMSET mail:email:<adresse>:INBOX:<uid>` dont le champ `data` pèse **160 Ko à 1,47 Mo** (corps texte + HTML complet, sérialisé) pour **11 à 16 ms** chacun. Quelques-uns par seconde suffisent à faire dépasser les 500 ms du budget du client aux **petites** lectures en file derrière (`user:id:*`, `usersettings:*`, résumés). Le cache n'était pas plein — 2,0 à 2,8 Go pour ~14 500 clés — il était **occupé**. Effet croisé mesuré : 38 attentes et 18 refus de contre-pression du journal d'audit sur `Spill buffer unreachable` alors que la borne du spill n'était qu'à 70 000 sur 120 000.
- **Livré** : `SizeBoundedCacheService` **décore** le cache enregistré par `AddSdk` et refuse toute charge utile au-dessus de `Cache:MaxEntryBytes`. Une entrée refusée se lit comme une entrée absente ; tout appelant traite déjà une absence comme « va la chercher » — pour `mail:email:*` c'est la lecture de `MailContents`, chemin normal depuis task-273. **Aucune perte fonctionnelle** : on échange une lecture en base contre la latence de queue de toutes les autres clés de l'instance.
- **⭐ LE SEUIL EST DÉRIVÉ DE LA MESURE, PAS CHOISI ROND** : le plus petit blocage du slowlog est **160 Ko à 11 ms**. À **64 Kio** l'ordre de grandeur est **~4 ms**, sous le plafond de 5 ms que le protocole de re-mesure de la US fixe lui-même pour `mail:email:*`. 128 Kio — le haut de la fourchette que le PO autorisait — resterait vers 9 ms et **manquerait ce critère** : sur une instance mono-thread partagée avec le spill d'audit, le cache d'identité et les marqueurs de purge, c'est le bas de la fourchette qui tient. Le défaut vit dans `CacheOptions`, **pas** dans le JSON : retirer la section ne débride pas le cache.
- **Deux options de la US écartées, et chiffrées** : (1) cacher le message **sans** ses corps aurait créé deux formes d'une même entrée et un risque de régression d'affichage, que la US interdit explicitement — le repli base existe déjà ; (2) **compresser** un corps de 1,5 Mo le ramène à ~300-400 Ko, soit encore 3 à 6 fois la borne : la commande resterait bloquante. L'objectif est la borne de taille, pas le tassement.
- **⭐ LA DÉCORATION EST LE BON NIVEAU, ET LE SLOWLOG LE PROUVE DÉJÀ** : la borne appartient à qui connaît la taille sérialisée, c'est-à-dire la frontière du cache — pas `MailController.GetEmail`. Le slowlog du 09/09 nomme **une seconde** entrée volumineuse (`usersettings:*`) : un correctif dans le seul appelant connu l'aurait laissée sans protection. Le budget de 500 ms et le disjoncteur du Sdk sont **intacts** (contenu attendu n° 4 de la US) — ils sont le symptôme, pas la cause. `HealthPlatform.Host.Sdk` étant consommé en NuGet depuis un autre dépôt et les `Repos` de la task ne listant qu'`api-mail`, le modifier aurait de toute façon violé la règle 6.
- **⚠️ DÉCORATION ÉCRITE À LA MAIN** : `Scrutor` n'est pas une dépendance du dépôt. La **durée de vie de l'enregistrement d'origine est reprise telle quelle**, jamais figée en singleton — une bascule du Sdk en scoped ferait sinon capturer un service scoped par un singleton, ce que le conteneur ne signale qu'à l'exécution. Sans cache enregistré, la méthode ne fait rien plutôt que d'inventer un enregistrement.
- **Métriques** : `mss_cache_entry_bytes` (histogramme) et `mss_cache_oversize_skipped_total` (compteur). L'histogramme porte ce qui est **OFFERT**, refus compris — un histogramme des seules entrées acceptées masquerait exactement la queue que la borne existe pour couper, et son p95 tomberait sur la borne le jour où elle se met à servir. Ni clé, ni corps, ni fragment de message en étiquette : la clé contient l'adresse MSSanté du praticien, c'est précisément pourquoi elle n'y est pas.
- **⭐ TROIS DÉFAUTS TROUVÉS AVANT LA PR, DONT DEUX QUE LA RELECTURE SEULE N'AURAIT PAS VUS** :
  1. **La mesure allouait ce qu'elle mesurait** (passe qualité, `6607c43`). `JsonSerializer.SerializeToUtf8Bytes(item).LongLength` alloue un tableau **de la taille de la charge utile** pour n'en lire que la longueur : **1,5 Mo sur le tas des grands objets à chaque refus**, c'est-à-dire sur le chemin même que la classe protège. La protection serait devenue la plus grosse allocation du chemin d'écriture, et le LOH n'est pas compacté hors collecte de génération 2 — exactement la classe de coût que task-194 avait réduite d'un facteur 13 sur ce profil de charge. Remplacé par un `IBufferWriter<byte>` compteur sur un tampon loué de 4 Kio : même octet près, **empreinte constante quelle que soit la taille du message**.
  2. **La mesure ajoutait un second point d'échec à l'écriture** (revue de code, `54dee31`). `Measure` n'attrapait que `NotSupportedException`, alors que le sérialiseur lève aussi `JsonException` sur un cycle d'objets. Le service interne enveloppe déjà ses écritures et rend `false` : ne rattraper qu'un des deux modes faisait du décorateur un **second endroit où une écriture de cache peut lever** — et il couvre *tous* les écrivains de cache d'api-mail, pas seulement `mail:email:*`. Un test de charge utile cyclique le garde.
  3. **`csharpsquid:S3604`** (Sonar, `006dbc8`) : initialiseur de champ dans une classe à constructeur primaire. Constructeur explicite — la borne est lue **une fois**, pas à chaque écriture.
- **Tests** : **13 neufs**. Les deux contre-épreuves qui comptent : **octets UTF-8 et non caractères** (un corps français est plein d'accentués ; mesurer sur `json.Length` laisserait passer près du double de la borne), et **le message lourd rendu complet à l'identique** aux deux lectures successives — corps texte, corps HTML et document rattaché compris.
- **✅ Sonar** : Quality Gate **OK**, 0 bug, 0 vulnérabilité, 0 hotspot sur le new code. `new_coverage` 91,5 → **91,6 %**, code smells 62 → **61**, ratings A/A/A. Les **2 smells new-code restants** (`CA1869` dans un test d'audit) **ne viennent pas de cette task** : hérités de `develop`, ils sont **déjà corrigés dans la PR #228** (task-295) en attente de merge — les recorriger ici produirait un conflit mécanique au merge des deux PRs. Convention `S3604` ajoutée à `conventions/csharp.md`.
- **⚠️ Deux tests de banc exclus du compte, et la raison est MESURÉE** : `GreenMailBenchSmokeTests` et `SmtpSessionReuseBenchSmokeTests` démarrent leurs propres conteneurs et échouent par intermittence sur ce poste, dont le banc de l'humain occupe cinq réplicas d'API et un PostgreSQL à 93 % de son cgroup. Le même commit a **passé puis échoué puis passé**. **Contre-épreuve décisive : le test échoue aussi sur `develop`**, sans aucun changement de cette PR. 4 361 tests réussis hors ces deux-là.
- **⚠️ Réserve assumée** : sur le chemin **accepté**, la charge utile est parcourue deux fois — une fois pour la mesurer, une fois par le service interne pour l'écrire. L'interface prend un `T` et le service interne possède sa sérialisation ; les octets ne peuvent lui être passés sans double encodage, et le Sdk est hors périmètre. Les charges acceptées sont sous la borne par construction, et sur le chemin refusé ce parcours **remplace** une sérialisation plus un aller-retour vers Redis.
- **⛔ Un item de DOD différé au test humain (HAG)** : tir journey 1000 iso avant/après — `Timeout getting key` ÷ 10 au moins, CPU Redis sous 0,5 cœur en pointe, slowlog sans `HMSET mail:email:*` > 5 ms, `Spill buffer unreachable` = 0 avec journal actif, étape 3 « Ouvrir un message enrichi » inchangée ou meilleure. Exige le banc distant, ~3 h par jambe.

---

### v1.75 — Les connexions Postgres croissaient avec le TEMPS, pas avec la charge : borner le drain du journal et nommer les routes — task-298

- **Task** : task-298 — `done`. **PR** : `api-mail` [#232](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/232) (`awaiting-human-merge`), branche `feat/task-298-borner-drain-audit-backends`, 4 commits. `dtos-mss` : 0 commit (auto-inclusion, aucun changement de contrat).
- **Origine** : jambe B de task-296 (2026-09-11, Postgres à 48 Go, donc **mémoire hors de cause**), code `develop` `d04f2ca`, 1000 bases hydratées, journal d'audit actif.
- **⭐ LA SIGNATURE QUI DÉSIGNE LA CAUSE** : backends Postgres **270 à 18h40 → 2 504 à 21h17**, **linéairement**, alors que le débit k6 plafonne à ~130 req/s dès 20h40. La croissance suit le **temps**, pas la charge — c'est la signature d'un idle long sur 1000 pools, pas d'une surcharge. Au plafond (`max_connections` = 2 500) : **118 886** refus « 53300 sorry, too many clients already ». Côté api-mail, Seq attribue 97 % des 53 456 exceptions à `AuditBackgroundService` ; le médecin n'en voit que la queue (67 × 503 via `GlobalExceptionHandler`).
- **⚠️ ET LE TIR PARAISSAIT PRESQUE VERT** : 0,024 % d'erreurs k6, p95 4,8 s. C'est précisément le piège — **12 % du trafic était délesté en régime**, et un trafic délesté **flatte** les latences servies. Les gains lus sur ce tir étaient des bornes optimistes, pas un verdict.
- **Deux termes s'additionnaient, et ils sont traités séparément** :
  1. **Le pooler** — `server_idle_timeout = 600` (porté de 60 à 600 par task-294 pour supprimer un churn de login qui coûtait 10-16 s sous pression mémoire, pression **levée** depuis par task-296). À 1000 bases, tout pool touché dans les dix dernières minutes garde ses 2-3 serveurs : borne dure 1000 × `max_db_connections` 3 = **3 000**, au-dessus de `max_connections`. → **600 → 120** (jambe 2). 120 s reste 2× au-dessus de la pause réelle entre deux gestes du médecin (~30 s à 1000) : **le churn que task-294 avait supprimé ne revient pas**, c'est pourquoi on ne retourne pas à 60. `max_db_connections` reste à 3 — **un seul facteur par tir**, sinon l'A/B ne prouve rien.
  2. **La route directe du journal d'audit** (task-292) — pool de 2 **par base**, mais **aucun plafond global** : un rejeu de spill parcourant les 1000 bases ouvrait jusqu'à 1000-2000 connexions directes en quelques minutes, en concurrence frontale avec le pooler. Mesuré **après** le tir : 2 500 backends en 3 minutes, 27 575 refus, administration Postgres refusée, **VM Docker figée 45 minutes** — troisième occurrence de la journée. → `Audit:DrainMaxConnections` (défaut **16**).
- **⭐ LE PLAFOND EST PRIS AU SEUL ENDROIT QUI OUVRE UNE CONNEXION** : `PersistGroupAsync`. Tous les chemins du drain y passent — lot ordinaire, rejeu de spill, et persistance directe de secours quand le canal est plein. Un plafond posé plus haut (dans `PersistBatchAsync`, par exemple) aurait laissé le rejeu libre, c'est-à-dire **le chemin même qui a figé la VM**. Les traces **attendent** leur tour : le sémaphore fait patienter, il ne rejette pas — invariant task-292 conservé.
- **⚠️ CE N'EST PAS UN DOUBLON DE `DrainParallelism`** : ce dernier borne la concurrence **à l'intérieur d'un lot**, et vaut 1 par défaut. Il ne dit rien du nombre de bases touchées **sur une fenêtre** — or c'est cela qui faisait grimper les backends, chaque base laissant derrière elle son pool le temps de son `Connection Idle Lifetime`. Les deux bornes sont orthogonales et coexistent.
- **Durée de vie des connexions directes** : `Connection Idle Lifetime` de la route d'audit **60 → 30 s**. L'amortissement du login visé par task-292 reste acquis (le drain repasse sur la même base bien plus souvent que toutes les 30 s en régime), mais après un rejeu les backends directs redescendent en **moins d'une minute** au lieu de deux.
- **⭐ L'OUTILLAGE MESURAIT FAUX, ET C'EST CE QUI A DÉSIGNÉ LE MAUVAIS COUPABLE** : `observe.ps1` classait les backends par `client_addr` — « le pooler parle depuis le réseau Docker `172.x`, tout autre est direct ». Or **la route directe arrive par la passerelle Docker `172.x.0.1`** : elle était comptée « pooler ». Le 11/09, la colonne « directs » affichait **0** pendant que le drain saturait le serveur, et le rapport imputait l'incident au pooler. L'attribution se fait désormais par **`application_name`** — `mss-mail-audit`, `mss-mail-provision` — que le client pose lui-même et que Postgres expose : elle ne se devine plus.
- **`report.py`** : les deux routes directes publiées **séparément** dans « Coûts résidents », plus une ligne **backends max / `max_connections` (%)** et un verdict **ROUGE explicite** « Postgres à `max_connections` » au-delà de 98 %. Un tir au plafond ne peut plus être lu comme un tir sain.
- **Tests** : **10 neufs**. 3 sur le plafond de drain (dont « 40 groupes, `DrainParallelism = 40`, plafond 4 → pic ≤ 4 » : le plafond doit **primer** sur la parallélisation demandée, sinon il ne borne rien), 3 sur l'attribution des routes, 4 sur le **rejeu en fixture** de la saturation du 11/09 — fixture synthétique reconstruite depuis les chiffres publiés, selon la convention du dépôt (`observe-login-retry.csv`), le CSV brut du banc n'étant pas versionné. **RED vérifié** : plafond neutralisé → 2 des 3 tests de drain tombent.
- **⚠️ LES ASSERTIONS PORTENT SUR LA CONCURRENCE OBSERVÉE, JAMAIS SUR UNE DURÉE** : un test qui mesurerait un temps serait un test de la machine. Le dépôt substitué retient chaque appel le temps que la concurrence soit visible et enregistre le pic.
- **Passe qualité** : retrait d'un compteur de pic **écrit et jamais lu** — code mort, et écriture de propriété non synchronisée depuis plusieurs threads. Les tests observent la concurrence côté dépôt, ce qui mesure mieux : ce qui atteint réellement la base.
- **Sonar** : Quality Gate **ERROR** — `new_violations` 155, `new_security_hotspots_reviewed` 83,3 %. **⚠️ Aucune de ces violations n'est attribuable à ce diff**, vérifié issue par issue : les 2 de `AuditBackgroundService.cs` (S103) sont aux lignes 373 et 438 alors que le diff touche 33-53, 485-490 et 517-523 — elles ont **quand même** été corrigées ; les 153 autres sont dans `report.py` (S3776, **liste noire** → `/sonar-s3776`), `journey-model.js`, `journey.js` et une quarantaine de fichiers de test non touchés. C'est le piège documenté : **la *new-code period* du projet englobe des tasks déjà mergées**. `new_coverage` **89,1 %** (seuil 80), duplication 0,048 %, ratings 3.0 / 5.0 / **A**.
- **Validation** : build 0 erreur, solution **4 376 tests, 0 échec** ; outillage de banc (`selftest.sh`, node + unittest) **363 tests, 0 échec**.
- **Documentation** : ADR-2026-07-27 (§ 4.x, le plafond atteint et ses deux termes) et skill `loadtest-skill` (pré-vol obligatoire : `pg_stat_activity` ≪ 2 500 avant le tir, < 2 000 avant `report.sh`).
- **⛔ Deux items de DOD différés au test humain (HAG)** — et ils ne peuvent pas l'être autrement : **jambe 1** (backends max < 2 300, 0 `53300`, `cl_waiting` < 5 % des relevés, rejeu post-tir sans « too many clients » ni redémarrage de VM) et **jambe 2** (backends max < 1 800, `08P01 = 0`, `cl_waiting = 0` soutenu, erreurs k6 ≤ 0,02 %). C'est **la jambe 2 qui rendra le premier verdict SLO opposable à 1000**. Chaque jambe exige un tir journey 1000 de plusieurs heures sur le banc distant, Postgres 48 Go, 1000 bases gardées.

---

### v1.76 — Une indisponibilité de la messagerie était invisible de bout en bout : l'analyse rend un résultat, les sessions se rétablissent, le médecin est prévenu — task-315

- **Task** : task-315 — `done`. **PRs** : `api-mail` [#243](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/243), `client-blazor` [#80](https://github.com/codengine-technologies/HealthPlatform.Client/pull/80), `client-mobile` [#76](https://github.com/codengine-technologies/HealthPlatform.Mobile/pull/76) (toutes `awaiting-human-merge`), `client-angular` **code-only**. Branche `feat/task-315-messagerie-indisponible-visible`.
- **Origine** : campagne de charge du 2026-09-16/17, passe d'hydratation à 1000 médecins. Rapport `Docs/audits/api-mail-loadtest-journey-500-audit-basecommune-20260917.md`.
- **⭐ LA SIGNATURE** : compteur `mssante_enrichment_message_duration_seconds_count` **gelé à 72 101 de t+130 min à t+210 min** (80 minutes), pendant que `http_server_request_duration_seconds_count` passait de 769 094 à **1 721 672** — **950 000 requêtes servies sans un seul message analysé**, les 5 réplicas publiant toujours leurs métriques. À la même minute, `mssante_imap_sessions_connected` chute de 2 224 à **0** et y reste, `_active` restant à ~2 200. 72 101 ÷ 98 ≈ **736 médecins**, exactement le front d'hydratation relevé en base (plein jusqu'à l'index ~640, nul au-delà de 700).
- **⚠️ ET L'ENDPOINT RÉPONDAIT 200** : `enrich/sync` continuait d'être appelé au même rythme (≈ 4 250 requêtes / 20 min), **zéro erreur**, en **2 à 3 secondes** — donc pas le court-circuit « déjà analysé », qui répond en ~25 ms. Contrôle nominatif Seq : `loadtest-815` appelle à t+170 min, reçoit `200` en 2 334 ms, finit le tir avec `MailContents = 0`. **264 praticiens sur 1 000** sans un document, un succès rapporté pour chacun.
- **Quatre défauts empilés** :
  1. **`EnrichEmailsAsync` rendait `Task`.** Le contrôleur ne pouvait journaliser que `uids.Count` — le nombre **demandé**. Les trois interfaces (`IImapService`, `IImapEmailFetchService`, `IBackgroundImapService`) portaient la même signature muette.
  2. **`FolderUnavailable` était connu du code, puis jeté.** `SingleMailFetch` et `EnrichmentChunkFetch` le transportent jusqu'à la boucle, qui s'en sert pour `break` — puis `fetched.Count == 0` conduit à un `return` muet.
  3. **Le chemin asynchrone ment sans recours.** `EnrichEmailsBackgroundAsync` journalisait `[Enrich] Completed {uids.Count} (async)` inconditionnellement, le `202 Accepted` étant déjà parti : **aucun appelant à qui rendre une erreur**. C'est le chemin que `client-blazor` utilise par défaut.
  4. **`CanAccessImap => IsOnlineMode`** — un drapeau de **mode**, pas un **état**. `connection/status` a répondu `canAccessImap: true` pendant toute la panne. **⚠️ Ce quatrième défaut n'est PAS corrigé par cette task** — voir le tir de vérification ci-dessous.
- **⭐ LE PRÉCÉDENT EXISTAIT DÉJÀ, DANS LA MÊME MÉTHODE** : task-293 avait posé un `UnavailableException` conduisant à un 503 en fin d'`EnrichEmailsAsync` pour les échecs d'extraction, avec ce commentaire — « le 200 qui a masqué l'incident du 2026-09-08 ». Le correctif de task-315 est **symétrique d'une décision déjà prise et acceptée** ; elle ne couvrait simplement pas la connexion perdue.
- **`EnrichmentOutcome`** — quatre comptes (demandés / déjà analysés / analysés / injoignables) et **non un booléen** : « déjà analysé » est un **succès** (le court-circuit fonctionne), « injoignable » un **échec**, et c'est leur fusion qui a masqué l'incident. `NothingReachable` porte la forme qui doit devenir un 503.
- **`enrich/async`** — log au compte réel, compteur `mssante_enrichment_unreachable_messages_total`, et **remise en file bornée** : 3 essais, retard croissant (30 s par tentative), **lot complet rejoué** car les déjà-analysés court-circuitent (idempotent, pas d'état partiel à transporter). Bornée dans les deux sens volontairement — une reprise non bornée sur une messagerie durablement morte transformerait le remède en tempête de reconnexions.
- **Sessions** — le balayage évince celles dont la voie IMAP est ouverte et le lien tombé (`IsImapLinkDead`), la requête suivante en recrée une connectée. « Reconnecté au prochain usage » **sans machinerie de reconnexion dans le chemin chaud**.
- **⛔ LE POINT 3 (SONDE DE SANTÉ) A ÉTÉ RETIRÉ LE 2026-09-17, INFIRMÉ PAR LE TIR.** Deux itérations de conception, puis un retrait :
  1. La première version testait `ActiveSessionCount > 0 && ConnectedSessionCount == 0` et aurait déclaré la messagerie morte **au démarrage** — une session qui n'a pas encore ouvert de voie IMAP est légitimement active et non connectée. Révélé par le test d'invariant du balayage, **en contredisant son voisin**. D'où `DisconnectedSessionCount`.
  2. **Le tir de vérification l'a démentie aussi.** Coupure provoquée du serveur de messagerie (API Toxiproxy) sous ~600 sessions : `canAccessImap` est resté **`true` pendant toute la panne**. Cause établie par l'expérience — une tentative d'analyse pendant la coupure laisse `active = 0`, car **quand la connexion ne peut pas s'établir, aucune session n'est retenue** ; donc `Disconnected = 0`, et la condition n'est jamais vraie.
  3. **Le défaut est structurel** : l'éviction des sessions mortes — celle qui fonctionne, 617 évincées en une minute — **supprime exactement la preuve** que la sonde cherchait. Les deux ne peuvent pas être vraies ensemble.
  4. **⚠️ Les six tests qui gardaient ce comportement passaient tous** : ils substituaient `DisconnectedSessionCount` à 2 200, une valeur que la réalité ne produit jamais dans ce scénario. Ils validaient le **modèle**, jamais le **monde**.
  Retiré : la condition (retour à `CanAccessImap => IsOnlineMode`, aucune régression), 4 tests unitaires, 2 tests d'intégration. `DisconnectedSessionCount` **conservé** (il sert l'invariant du balayage, lui prouvé). Le bon signal — **issue des tentatives récentes**, pas inventaire des sessions — demande d'instrumenter le chemin de connexion, sans point de passage unique aujourd'hui : **US de suivi**.
- **⭐ CE QUE LE TIR A PROUVÉ, ET QU'AUCUN TEST NE POUVAIT DONNER** (2026-09-17, coupure provoquée, ~600 sessions, branche de la task) : pendant la coupure, `enrich/sync` rend **`HTTP 503` `application/problem+json`** en 2,21 s — « La messagerie est momentanément indisponible : 5 message(s) sur 5 n'ont pas pu être analysés et restent à traiter. Ils le seront dès son rétablissement. », sans hôte ni dossier ni UID ; le compteur d'enrichissement **gèle** à 21 584 ; les sessions passent de 616 connectées à 0, puis **617 actives → 0** en une minute (éviction). Après rétablissement, **même appel, même boîte, même session, sans redémarrage** : `HTTP 200`, `analysed: 5`, compteur **21 584 → 21 589**. Le 2026-09-16, ce même compteur était resté figé **80 minutes**.
- **Fronts** — le 503 n'aurait atteint personne : Angular souscrivait **sans handler d'erreur** (`mail-list.component.ts:222`), Blazor **avalait** l'exception dans un `catch` (`MailService.cs:40-66`), mobile avait la même forme qu'Angular. Et `canAccessImap` n'était **déclaré que dans un type** (`sync.model.ts:31`), consommé nulle part. Message unique sur les trois : « Messagerie momentanément indisponible. Les documents seront rattachés dès son rétablissement. » — non bloquant, **une seule fois par épisode**, liste jamais vidée, drapeau retombant au premier succès.
- **⚠️ `PostAndGetStatusAsync` (Blazor)** : `PostAsync<T>` rend `IsSuccessStatusCode`, où un 503 est indistinguable d'un 500, d'un 400 ou d'un « non authentifié » — l'appelant ne pouvait rien dire de précis. Placé **avant** les surcharges `PostAsync` pour rester adjacent (S4136).
- **`OutageNotificationGate` (Blazor)** : la règle « une fois par épisode » isolée en unité **pure**, parce que `HttpRequestService` est une classe concrète à méthodes non virtuelles, donc non mockable. Extraire la règle est ce qui la rend éprouvable.
- **Tests** : **17 neufs**. `MailServerUnavailabilityTests` (8), 3 tests de contrôleur, `OutageNotificationGateTests` (5), `mail-outage-notifier.service.spec.ts` (5), 3 specs Angular. **RED vérifié par mutation** : garde du contrôleur retirée, `EnrichEmailsSync_NothingReadWhileWorkRemained` échoue sur `Assert.Throws() Failure: No exception was thrown` — le 200 du 16/09.
- **Validation** : `api-mail` **4 507 tests** (183 + 492 + 2 488 + 837 + 507), `client-blazor` **260**, `client-mobile` **861**, `client-angular` **380** plus `nx build mss`. Zéro échec.
- **Sonar** : **zéro itération, aucune issue introduite** — vérifié **fichier par fichier** et non au total. Les 2 issues des fichiers touchés sont préexistantes (`ImapService.cs` S3604 **ligne 83**, `BackgroundImapService.cs` S107 **ligne 60** ; les diffs portent sur 1341-1490 et 180-290). Code neuf de la task couvert à **100 %** (`EnrichmentOutcome.cs` 6 lignes / 0 non couverte, `ConnectionModeService.cs` 10 / 0). Quality Gate **ERROR** sur **une seule** condition, étrangère à la task : `new_security_hotspots_reviewed` 0 %, hotspot `TenantRegistryMigrator.cs:106` (log-injection, chantier registre task-299/300), revu manuellement par l'humain selon `sonar-targets.yml`.
- **Lint** : Angular 28 erreurs ramenées à **0** en une itération (toutes `prettier/prettier` sur le seul fichier de la task, re-indentation du `subscribe`) ; **auto-fixer restreint à ce fichier**, l'humain ayant 11 fichiers non commités dans ce repo. Mobile : `All files pass linting`, zéro itération.
- **⚠️ `/verify-visual` sauté — outillage absent** (`Tools/visual-verify/` n'existe pas dans le workspace). **Ce que le skip laisse non vérifié** : `inbox.page.ts` est une **page**, et l'écran blanc est la seule sévérité bloquante de cette étape ; ni les 861 tests ni le build ne le couvrent. Renvoyé à l'étape 5 du plan de test manuel.
- **⚠️ Deux suggestions de revue, non bloquantes, sur `BackgroundImapService.cs`** : (1) `Unreachable: pendingUids.Count - processedCount` compte aussi les UID **absents du serveur** (mails supprimés, retirés par `RemoveMissingUidsAsync`) — `ImapService` garde le sien derrière une condition sur `folderUnavailable`, ce chemin-là ne le fait pas ; **latent**, le seul appelant (`BackgroundSyncService:427`) ignore le résultat, mais à corriger avant que quiconque s'y branche ; (2) le littéral d'`EnrichmentOutcome` apparaît **5 fois** — la passe `/simplify` a factorisé ce motif dans `ImapService` et manqué celui-ci.
- **⭐ LE 503 NOMME SA CAUSE** (ajouté le 2026-09-17, sur question de relecture — *« un ZIP corrompu casse-t-il la chaîne ? »*). **Non** : `IheXdmProcessingService.Classify` sépare `InvalidArchive` (`FormatException`, `InvalidDataException`, `MimeKit.ParseException`) — le message est **enregistré sans ce document**, rien n'est interrompu, aucun 503 — de `ScratchUnavailable` / `Io`, qui laissent le message à traiter. `PersistEnrichedBatchAsync` ne teste que `HasTechnicalFailure`, donc une archive invalide ne prend pas le `continue` et n'affecte aucun autre message du lot. **Mais la vérification a révélé autre chose** : deux causes très différentes rendaient le **même 503** sur cette route — messagerie injoignable (task-315) et extraction en échec technique (task-293) — et les trois fronts ne lisaient que le **code HTTP**. Sur une panne de disque, le médecin lisait « Messagerie momentanément indisponible » **alors que sa messagerie fonctionnait**. Correctif : `UnavailableException` porte un code, estampillé par le `GlobalExceptionHandler` dans le champ **`code`** du ProblemDetails (`MAIL_SERVER_UNAVAILABLE` / `DOCUMENT_PROCESSING_UNAVAILABLE`). **Le `detail` distinguait déjà les deux cas et a été écarté comme discriminant** : c'est une phrase, donc traduisible et reformulable — y coupler trois fronts, c'est garantir qu'une reformulation casse l'affichage sans que personne ne le voie. Les fronts rendent un message par cause, un message **neutre** quand elle n'est pas nommée, et **reparlent si la cause change** dans un même épisode (se taire laisserait le médecin sur une explication devenue fausse). Tests : 2 d'intégration backend, 9 sur la porte Blazor, 3 mobile, 3 Angular — dont les **contre-épreuves d'attribution**, sans lesquelles un code constant passerait pour une distinction.
- **⚠️ ANGLE MORT ASSUMÉ, renvoyé à la US de suivi** : `MailListComponent.razor` (Blazor) emprunte le chemin **asynchrone**, qui rend `202 Accepted` immédiatement — un médecin qui ouvre un dossier pendant une panne **ne voit rien**, seul le widget de notifications déclenche le bandeau. Angular et mobile sont couverts sur l'écran principal.
- **Périmètre explicitement exclu** : `enrich/sync` **n'est pas** mis en file (le chemin reste synchrone ; seul `enrich/async` gagne une reprise bornée) ; la **cause** de la déconnexion du 16/09 n'est pas instruite (hypothèse d'un redémarrage du pod Dovecot/Toxiproxy, invérifiable — `kubectl` refusé sur ce poste — et **sans objet** : quelle qu'elle soit, l'application doit la voir et s'en remettre) ; la chauffe du banc, qui se déclarait réussie sur un code HTTP alors que 264 médecins n'avaient rien reçu, est un **finding de harnais séparé**.

### v1.77 — Le banc mesure des praticiens inscrits, pas des VUs en boucle : scénario `terrain` (absence → session → geste), profils, concurrence émergente — task-321

- **Task** : task-321 — `done`. **PR** : `api-mail` [#245](https://github.com/codengine-technologies/HealthPlatform.Api.Mail/pull/245) (`awaiting-human-merge`). Branche `feat/task-321-scenario-terrain`. Harnais k6 + Python + docs uniquement : **aucun fichier C#**.
- **Origine** : tir `journey-1000-hydrate-20260917` — premier palier 1000 sur population entièrement hydratée (`Docs/audits/api-mail-loadtest-journey-1000-hydrate-20260917.md`). Le rythme injecté a été **mesuré** sur la fenêtre de régime : **un passage complet toutes les 87 s par médecin, 8,2 requêtes/minute, 1 000 médecins tous actifs en même temps pendant 3 h 30** — 41 ouvertures de messagerie par heure et par praticien. K valait 1 (aucune compression du temps de réflexion) : l'irréalisme n'était pas dans la vitesse des gestes mais **au-dessus** — le VU ne quittait jamais la messagerie, et la concurrence était fixée à 100 %.
- **Le modèle — trois couches** : *absence* (le praticien consulte ; log-normale, dominante), *session* (il ouvre la messagerie et enchaîne des passages tant que la durée tirée n'est pas épuisée, au moins un), *geste* (`journey`, **inchangé** — les temps de réflexion par geste étaient déjà à la bonne granularité). **Profils** attribués de façon déterministe à chaque VU (suite à faible discrépance : rejouable, entrelacé, parts respectées à ± 2 %) : occasionnel 30 % (2 sessions/j × 3 min), régulier 50 % (5 × 6 min), intensif 20 % (10 × 12 min), journée de 8 h. L'absence moyenne se déduit (`journée ÷ sessions − session`) : un régulier s'absente ~90 min. La première absence d'un VU est uniforme sur [0, moyenne] — sinon la première session serait une rafale de 1 000 dashboards qui n'existe pas sur le terrain.
- **Ce que ça change dans ce qu'on mesure** : **N VUs redevient « N praticiens inscrits »**, la concurrence active **émerge** (attendu aux défauts : ~4-8 % des inscrits). La grille SLO par geste est **identique** ; le verdict d'un palier est titré « **N praticiens inscrits — M actifs en moyenne** », jamais N seul — sans M le chiffre n'a pas d'unité. Nouvelles grandeurs publiées : concurrence active moyenne (Σ secondes de session ÷ fenêtre de régime), sessions et passages par praticien et par heure, **obtenus ET attendus** — le modèle est fermé, donc un serveur lent allonge les sessions, *augmente* les actifs et *réduit* les passages/h : l'écart se lit dans ce sens.
- **Hypothèses, pas mesures — et la calibration est déjà instrumentée** : les profils par défaut sont construits par cohérence (une journée de médecine générale ≈ 25 consultations de 15-20 min), surchargeables (`TERRAIN_PROFILES`, `TERRAIN_DAY_HOURS`), archivés avec le tir. Le journal d'audit (`audit_traces`, task-300/301) porte exactement ces distributions par praticien et horodatées (`MailboxSessionOpened`, `MailRead`, `MailSend`, `AttachmentDownload`) : en production, une requête remplacera les profils inventés par les profils **observés** — décision produit, par le PO.
- **⭐ La chauffe sur population hydratée** (`JOURNEY_WARMUP_HYDRATED=1`) : l'allocation (`warmupWindowSeconds`) et les vagues (`warmupWaveDelaySeconds`) étaient dérivées **a priori** du plafond d'analyse (9,5 msg/s, 8 médecins simultanés — task-253/264), constantes d'une population **froide**. Sur une population déjà analysée, l'enrichissement court-circuite en ~25 ms/lot (task-315 : `AlreadyAnalysed`) — et l'allocation restait froide : **9 807 s de chauffe (78 % de la fenêtre) pour 12 018 messages réellement analysés sur 118 270 demandés**. La déclaration substitue le coût du court-circuit (lot ÷ 0,025 s), monte la concurrence à 200 ; à 1000 praticiens l'allocation passe de ~10 300 s à **< 5 min**. Sans la variable, valeurs **strictement inchangées** (témoin testé). Frontière avec task-254 (portillon de l'analyse réelle) écrite dans le code.
- **⭐ Le débit émergent du palier était sous-estimé d'un facteur 5,5** : `report.py` divisait les requêtes taguées `palier:N` par la fenêtre **entière** du palier, chauffe comprise, alors que `palierAt` garantit qu'aucune requête du palier n'a lieu pendant l'allocation. Le 17/09 : **24,66 req/s publiés** (310 766 ÷ 12 600) pour **136 réels** (÷ 2 284 s de régime) — ce qui rendait le palier 1000 « plus calme » que la moyenne globale du tir (100 req/s), une absurdité. Corrigé pour `journey` comme pour `terrain` ; le cas est rejoué chiffre pour chiffre dans `test_report_terrain.py`.
- **Implémentation** : `lib/terrain-model.js` (module pur : parseur à refus francs — parts, noms littéraux pour les tags, doublons, plancher d'absence de 60 s ; K délibérément tenu à 1 sur absence et session ; rythme attendu ; passage absence comprise) ; `checkBudgets(iterationSecondsOverride)` — sans lui le budget exigeait les réserves d'un praticien à 41 passages/h et **refusait** un tir 1000 × 3 h que `terrain` accepte aux réserves du seed standard (les deux cas testés) ; `journey.js` : `doctor()` bifurque en tête, le chemin journey est textuellement l'ancien, `passage()` isole les gestes, métriques `terrain_*` par palier, tag `profile` posé **une seule fois** (métrique du harnais, jamais `http_req_duration`), contexte archivé (`sessionModel`, `terrain`, `warmupHydrated`) ; `run.sh terrain` ; `report.py` : famille distincte dans l'INDEX (`🚶(fermé, inscrits)`, colonne VUs « N (≈M actifs) »), section « Terrain », titre du verdict, ligne « chauffe en mode population hydratée », helpers rendant `None` quand la métrique manque — jamais 0.
- **Tests** : 20 (`terrain-model.test.mjs`) + 4 (`warmup-hydrated.test.mjs`) + 10 (`test_report_terrain.py`) — **témoins négatifs systématiques** (journey sans terrain, sans variable hydratée, sans `warmupEndS`). `k6 inspect` : 14 seuils `terrain_*` en mode terrain, **0** en journey, refus franc des parts fausses. `selftest.sh` : 121 node + 373 unittest verts. `api-mail` build 0 erreur, **4 493 tests** verts (16 skips préexistants).
- **Sonar** : **skipped, explicitement** — api-mail touché sans aucun fichier C# ; le scanner .NET n'aurait rien à lire, un scan complet coûterait ~15 min pour une table identique à la baseline. Lint Angular/mobile et vérification visuelle : skipped (repos non touchés ; 2 fichiers Angular non commités **antérieurs** à la task, hors périmètre).
- **⚠️ Ce que cette US ne fait pas** : elle ne fournit **aucune** mesure terrain — elle fournit l'instrument et ses hypothèses. Sur un tir de 3 h, un praticien fait 2 à 5 sessions : les percentiles se lisent sur la population, pas par profil. Le premier tir `terrain` à 1000 est lancé dans la foulée (2026-09-18) : ses conclusions iront dans un audit séparé.
...
|---|---|---|
| task-276 | **L'instrument publiait une pointe comme une valeur d'exploitation, et une US allait s'écrire dessus.** Le tir discriminant de task-270 avait conclu que la fusion du chemin de lecture de dossier allongeait la section critique du verrou (`ReadFolder` 11,871 s au p95, voisins ×19). **Deux défauts empilés dans la table** : fenêtre couvrant tout le tir chauffe comprise (alors que task-264 sépare les deux partout ailleurs), et surtout **réducteur `max`** — le pire instant imprimé comme une valeur d'exploitation, `11,871 s` et `11,871 s sur 2,8 % des relevés` s'imprimant à l'identique. Le second n'était pas au périmètre initial : trouvé **en exécutant** l'étape « établir sur pièce », et c'est lui qui portait la fausse conclusion. Livré : `regime_window()` (palier nommé dans le titre), `collect_prometheus_regime()` (borné aux séries `imap_session`), `_series_distribution()` (médiane/p90/pointe/part > 2 s), attribution **symétrique** (nomme une pointe, refuse d'excuser une détention soutenue). Piège traité : les `rate(...[1m])` font entrer 60 s de chauffe au premier point d'une sous-fenêtre — **en petite quantité, donc sans se voir** ; `PROM_RATE_LOOKBACK_S` décale. **⭐ Conclusion du tir renversée** : médiane de `ReadFolder` **0,469 s**, INFÉRIEURE aux 0,692 s de `GetFolderQuery` qu'elle remplace — task-270 a *raccourci* la section critique ; les 11,871 s sont un transitoire (2,8 %), et la même forme frappe `GetEmailContent` (5,4 %) et `GetAttachmentStream` (11,6 %), jamais touchés par task-270. **Le remède prévu n'a pas été écrit, et c'est le résultat de l'US** (issue explicitement prévue par le task file ; l'écrire aurait répété task-222, en plus insidieux — cause *mesurée* mais *mal réduite*). Finding neuf scopé : `GetAttachmentStream` seule détention tenue en régime (p90 2,136 s). **Quatrième piège de la famille** (task-211 buckets, task-245 plafond, task-271 saturation) et **la première où l'artefact avait déjà produit une conclusion publiée**. Code review : 1 défaut trouvé par elle-même (table à en-tête sans ligne = « détentions nulles » au lieu de « aucune mesure », mode d'erreur task-214). 333 tests verts. ⚠️ Sonar non exécuté (`$SONAR_TOKEN` absent, port doc 9001 vs serveur 9000) — aucun vert revendiqué. | — |
...
| task-273 | **⚠️ Livraison jumelle de task-270, plus la pollinisation.** Cause du coût de l'appel `folder` à l'arrivée dashboard **établie par phase avant remède** (verrou ×2 avec fenêtre de préemption, 7 RTT dont 2 redondants, base hors de cause, matérialisation négligeable ; « ouverture gratuite » vs « appel dominant » réconciliés : cache-hit vs chemin froid). Fusion du chemin froid ré-implémentée indépendamment de #205 (recouvrement découvert après ouverture de la PR #206 — arbitrage dans `questions/task-273.md`, ne pas merger les deux en l'état). Apports propres : pollinisation LIST-STATUS → `folder:status` de tous les dossiers (l'appel `folders` arme la validation du quatuor quel que soit l'ordre), tests d'intégration DOD (2ᵉ arrivée = zéro sollicitation ; nouveau message servi à l'expiration de la fenêtre), famille de verrou dédiée `GetFolderRead`. Aucun TTL modifié, contrat inchangé. Quality Gate OK, 1 seule issue new-code attribuable (CA1861, corrigée, récidive comptée). | — |
| task-281 | **Le chemin d'archivage comptait zéro aller-retour.** `AppendToSent` était le seul chemin IMAP sans aucune sollicitation enregistrée — d'où trois US (276, 277, 278) incapables d'attribuer une attente de 3,938 s de p95 médian derrière lui. Décomposition établie (4 allers-retours sous le verrou, plus CONNECT+TLS+AUTH à froid, et une résolution de dossier non mémoïsée), instrument livré sur le patron task-262, **aucun remède** — le piège task-213/215/216 est évité et la mesure qui en désignerait un n'existait pas encore. 3 tests RED→GREEN, suite 3 932 vertes. Sonar exécuté : 0 issue et 0 hotspot sur le diff. Chiffres de répartition reportés au tir de clôture (rapport de référence git-ignoré, absent du clone). | — |
| task-289 | **Un flag absent de l'environnement éteignait l'étage IA en entier.** `RefreshAsync` évaluait les 11 flags dans une seule boucle et le client Flagsmith lève sur un flag inconnu : la première absence avortait le rafraîchissement entier, aucun snapshot n'était publié, et les 11 flags retombaient sur leur repli — fail-open pour les widgets, **fail-closed pour les `ai_*`**. D'où 404 sur tous les résumés cliniques à cause d'un flag de widget. Livré : isolation par flag, statut `partial` avec throttle dédié, log de dérive qui **nomme** les flags absents, log de panne véridique (« serving last known flag state » n'est plus écrit à froid), symétrie sur le chemin d'identité, amorce au démarrage avec `Task.Yield`. **Piège d'héritage vérifié par réflexion** : `FlagsmithAPIError` hérite de `FlagsmithClientError`, donc un catch nu détruisait l'invariant « dernier état connu » de task-199 — attrapé par un test de non-régression, pas par la relecture. **Deux tours de revue en CHANGES REQUESTED**, le second trouvant un défaut que le premier avait causé : l'heuristique censée corriger un libellé trompeur **mentait** dès que les valeurs servies coïncidaient avec les replis (Flagsmith répond, widgets ON et `ai_*` OFF) — retirée, le remède est de ne rien affirmer. Contre-épreuves : liste vidée → 2 tests tombent, isolation neutralisée → 7, niveau de log muté → 3, heuristique réintroduite → 2, `ai_pipeline` en fail-open → 6. QG **OK** 5/5, **0 issue sur les 15 fichiers**, 4 026 tests verts. Réserve : `new_coverage ≥ 95 %` non opposable (`new_lines` > `ncloc`), couverture établie fichier par fichier. Le seul finding du cycle (CA1859) avait été **introduit par la passe qualité** → `conventions/csharp.md` incrémenté, et `conventions/` enfin versionné (il était ignoré, la boucle d'auto-amélioration se réinitialisait en silence). Suite : `todo-task-290.md` (mails passés à côté de l'IA, non marqués donc non rattrapables en l'état). | — |
| task-294 | **Le multiplexeur PostgreSQL fait attendre au lieu de rejeter.** À 1000 praticiens PgBouncer répondait `08P01 connect timeout` (646 refus au drain sérialisé, 15 873 au drain parallèle) au lieu de faire attendre, avec `sv_login` à 212-243 — la signature d'une réouverture permanente des 1000 pools. Profil de banc : `server_idle_timeout` 60 → 600, `server_lifetime` 600 → 3600, `server_connect_timeout` 15 → 30. Côté application, le refus devient une **erreur typée** : `08P01` / `08006` / `57014` cherchés dans toute la chaîne d'`InnerException` (la stratégie EF Core enveloppe le driver) et rendus en 503 `ProblemDetails` sans fuite. Le non-rejeu du chemin de données est **prouvé par un test**, pas déduit de la configuration. Le harnais lit désormais les refus par delta et classe **ROUGE** tout total non nul — un refus est un 500 servi en ~10 ms, et il flatte les latences des demandes servies. | — |
| task-295 | **Le banc lit le coût du login PostgreSQL au lieu de le déduire.** Les trois grandeurs qui portaient le diagnostic du `08P01` étaient relevées **à la main pendant le tir** (un `psql` chronométré à 10-16 s contre 6 ms sain, `sv_login`, le cgroup à 99,9 %) : le verdict rouge était une reconstitution après coup. Trois sondes séparées les publient maintenant sur la même grille de temps que `cl_waiting` et les refus — coût d'un login mesuré **dans** le conteneur, débit de **création** de backends (`started_last_60s`, à opposer au débit de requêtes), survie à l'idle (témoin du churn), origine pooler/directe, pression mémoire du cgroup. Le rapport avertit sans classer ROUGE (un login lent **sans** refus est une alerte, pas une panne) et **corrèle** en trois branches : cause probable / cause ailleurs / corrélation impossible faute de sonde. Deux défauts trouvés avant la PR : un chronomètre qui rendait 0 sur l'image Alpine, et un filtre qui redéfinissait en silence un compteur comparé depuis task-204. | — |
| task-297 | **Le cache partagé cessait d'être disponible parce qu'il portait des corps de message entiers.** Redis exécute une commande à la fois : le slowlog du tir 1000 du 2026-09-09 porte des écritures de 160 Ko à 1,47 Mo coûtant 11 à 16 ms, derrière lesquelles les petites lectures dépassaient le budget de 500 ms — **6 568 délais d'attente sur un tir sans aucune autre charge Redis**, et jusqu'aux refus de contre-pression du journal d'audit, qui n'y était pourtant pour rien (le A/B « journal désactivé » l'a exonéré). Un décorateur posé **à la frontière du cache** (et non dans l'appelant connu : le slowlog nomme déjà une seconde entrée volumineuse) refuse toute charge utile au-dessus de 64 Kio — seuil **dérivé de la mesure**, pas choisi rond : 160 Ko coûtent 11 ms, donc 64 Kio en coûtent ~4, sous le plafond de 5 ms que le protocole de re-mesure fixe. Rien n'est perdu : une entrée refusée se lit comme absente et l'appelant retombe sur la base. Deux instruments publient enfin la distribution des tailles. Trois défauts trouvés avant la PR, dont une mesure qui allouait 1,5 Mo sur le tas des grands objets **à chaque refus**, sur le chemin même qu'elle protège. | — |
| task-315 | **Une indisponibilité de la messagerie était invisible de bout en bout.** `EnrichEmailsAsync` rendait `Task` : `FolderUnavailable` remontait jusqu'à la boucle, servait à en sortir, puis mourait là — le contrôleur journalisait le nombre **demandé** et répondait 200. Mesuré au tir du 2026-09-16 : compteur d'enrichissement **gelé à 72 101 pendant 80 minutes** tandis que l'API servait **950 000 requêtes de plus**, `imap_sessions_connected` à 0 avec ~2 200 sessions actives, **264 praticiens sur 1 000** sans un document et un succès rapporté pour chacun. Livré : `EnrichmentOutcome` à quatre comptes, **503** `UnavailableException` sur `enrich/sync` (symétrique du 503 de task-293, dans la même méthode), reprise bornée sur `enrich/async` (le 202 étant parti, il n'y a plus d'appelant à qui rendre l'erreur), éviction des sessions au lien mort, et `CanAccessImap` branché sur l'état réel. Les trois fronts affichent un bandeau non bloquant disant que **rien n'est perdu** — aucun ne savait quoi faire d'un 503. **⛔ Le point 3 (sonde de santé) a été RETIRÉ le 2026-09-17**, infirmé par un tir avec coupure provoquée : `canAccessImap` restait `true` pendant la panne, car aucune session n'est retenue quand la connexion échoue — et l'éviction efface la preuve que la sonde cherchait. Les six tests qui le gardaient passaient, en substituant une valeur que la réalité ne produit jamais. Le même tir a **prouvé** les points 1 et 2 : 503 problem+json pendant la coupure, puis 200 et compteur 21 584 → 21 589 après rétablissement, **sans redémarrage**. Sonar zéro issue introduite, vérifié fichier par fichier. | — |

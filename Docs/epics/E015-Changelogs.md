...
### v1.19 — Le décompte des sollicitations du serveur de messagerie, et l'artefact de mesure qu'il a révélé — task-222

- **Task** : task-222 — `done` (US **re-cadrée sur la mesure** le 2026-08-04, cf. `questions/task-222.md`). **PR** : `api-mail` #150. Commits `458fc48` (décompte), `317a403` (correctif — **retiré**), `ee095f1` (simplify), `1c2b24c` (S125), `76ba6d3` (commentaire TOCTOU — retiré avec), `5da54bc` (**revert du correctif**). `dtos-mss` : branche vide, aucune PR, aucun publish NuGet.
- **Livré : le décompte des sollicitations.** `IMailServerSolicitationRecorder` (`Scoped`) + `MailServerSolicitationRecorder` (`ConcurrentQueue`) : compteur `mssante_mail_server_solicitations_total{command,operation}`, étiquette de trace `mss.mail_server.solicitations` posée par `GetEmailContentAsync` avec le delta **propre à l'appel**. Sites : `ResolveFolder` / `OpenFolder` / `FetchBodyStructure` / `FetchBodyPart` ×n / `CloseFolder` (`ImapService`), `Connect` / `Authenticate` (`ImapConnectionService`) — **une session reprise du pool ne compte pas**, donc borne inférieure **exacte** des allers-retours. **PGSSI-S** : étiquettes littérales (`MailServerCommands`, familles d'opération), même règle que `LockOperationFamily` (task-213) — aucun chemin de dossier, aucun UID, aucun nom de PJ.
- **Retiré : l'écriture en retour du contenu, et le motif est grave.** `317a403` avait ajouté `SaveMailContentAsync` appelée depuis `GetEmailContentAsync`, par symétrie apparente avec `GetAttachmentAsync` → `UpdateAttachmentAsync`. **La symétrie est fausse** : l'existence d'une ligne `MailContents` n'est pas un cache de corps, c'est le **marqueur d'enrichissement** — `GetEnrichedUidsAsync` (`MailContents.Any()`) consommé par `ComputePendingEnrichmentAsync` (`ImapService:827`) et `BackgroundImapService:142` ; `TryResolveExistingMailAsync:177` (`ContentCount > 0` ⇒ « already enriched — skipping », **avant** la branche de promotion) ; `GetCoverageCountsAsync` pour l'indicateur produit. Une ligne corps-seul faisait donc **sortir le UID de `pendingUids`** ⇒ CDA jamais décodé, aucun `MailMedicalDocument`, aucun rattachement patient, aucun résultat de biologie, aucun embedding — et `NotifyAlreadyEnrichedAsync` annonçait au frontend que c'était terminé, donc il cessait d'attendre. **Perte de contenu clinique, silencieuse, sans aucune erreur.** `Content` nul est un **état voulu** entre l'ouverture (affichage des premiers éléments) et la fin du décodage. Retiré aussi `HasDisplayableContent`, justifié par un « écran blanc définitif » qui n'avait pas été établi (l'enrichissement écrit contenu et documents dans le même `SaveChanges` — une ligne vide committée n'est pas un état démontré).
- **Comment le défaut a échappé à 3 399 tests verts** : aucun ne traverse la séquence **ouverture → enrichissement**. Le défaut vit dans l'interaction de deux chemins corrects isolément. Lacune de couverture signalée comme telle dans `questions/task-222.md` (question 3).
- **Garde-fous posés contre la réintroduction** : commentaire d'avertissement dans `IMailRepository` **à l'endroit exact** où la méthode vivait, avertissement en tête de la doc XML de `GetEmailContentAsync`, et test d'intégration `OpeningAMailNotYetEnrichedSolicitesTheMailServerAndStoresNothing` qui asserte contre un **vrai PostgreSQL** qu'une lecture ne crée **aucune** ligne de contenu.
- **Le diagnostic de la US est invalidé, et c'est le décompte qui l'a montré.** Le scénario `journey` **n'appelle jamais l'enrichissement** (son propre commentaire : « journey n'appelle jamais enrich ») et chauffe sa bande de relecture par `getEmailContent`, en commentant à tort que « le GET contenu matérialise le `MailContent` ». L'étape 3 « ouvrir un message enrichi (servi base) » mesurait donc des mails **jamais enrichis** : un fetch IMAP complet, comportement normal. D'où l'égalité exacte 440 vs 442 ms qui avait fondé la US. Les 34 ms de la PJ ne prouvent rien contre le produit : `GetAttachmentAsync` cache des **octets**, sans sémantique d'enrichissement. **Conséquence** : le verdict « étape 3 au rouge » du rapport `report-journey-certif-n200-180029.md` est **non opposable** tant que le harnais n'enrichit pas — au même titre qu'un chiffre à rythme accéléré.
- **Vérification demandée par la US sur `mssante_lock_hold_duration_seconds`** : soupçon de « même famille de défaut que task-214 » — **non confirmé**. `ImapLockScope.DisposeAsync` appelle bien `RecordLockHold(..., _tags.Family, _tags.Lane)` depuis task-214, et `report.py` interroge bien `sum by (le, operation) (...{lock="imap_session"}[1m])`. Le silence sur la fenêtre du tir est côté **observation** (fenêtre `[1m]` face à 3 111 itérations sur 35 min), pas côté instrumentation.
- **Tests** : 4 conservés (décompte), 8 retirés (verrouillaient le comportement retiré), 1 fichier supprimé (`MailRepositoryContentWriteBackTests`), 1 renommé (`MailContentServedFromStore…` → `MailServerSolicitationCountIntegrationTests`). `ImapServiceTests` : message enrichi ⇒ **0 sollicitation** et verrou de session non acquis ; contenu porté par les seuls documents médicaux ⇒ servi sans sollicitation ; mail non enrichi ⇒ la **séquence exacte** des 5 commandes est assertée. Intégration sur **vrai PostgreSQL** : les deux faces, dont la garde anti-régression ci-dessus.
- **Build / tests** : 0 erreur, 0 avertissement ; **3 389 verts**. **1 flaky pré-existant** : `Services/Export` (`MarkdownPdfRendererTests.RenderHeadingPreservesText` sur ce run, `MailExportServiceTests` sur le précédent — même famille `UglyToad.PdfPig` documentée en v1.18), vert en isolation, `Services/Export` **hors diff**.
- **Sonar** : **zéro dette introduite** (1 `S125`, faux positif de forme, converti en doc XML). QG `ERROR` avant comme après sur 18 `new_violations`, **aucune** dans un fichier du diff : 16 → task-220, 2 `S103` → task-185 / task-218.
- **Arbitrage rendu le 2026-08-04** : task-222 est **re-cadrée sur la mesure** — le décompte livré ici *est* la US, le correctif applicatif sort du périmètre et n'est pas rouvert (aucun défaut produit établi sur ce geste). **La correction du harnais passe à task-224**, qui l'inscrit comme son **défaut 5**, priorité relevée 3 → 2 : c'est le seul de ses cinq défauts à avoir faussé un **verdict** et non seulement une lecture, et il rend task-224 bloquante pour un chiffrage qu'elle ne portait pas. **Reste ouverte** : la lacune de couverture ouverture → enrichissement (question 3 de `questions/task-222.md`) — la garde posée ici bloque *ce* défaut, pas sa classe. task-222 passe en `done`, sa DOD re-cadrée étant satisfaite.
...
| task-222 | **Le décompte des sollicitations du serveur — et l'artefact de mesure qu'il a révélé.** Livré : `IMailServerSolicitationRecorder` (`Scoped`), `mssante_mail_server_solicitations_total{command,operation}`, étiquette de trace `mss.mail_server.solicitations` ; une session reprise du pool ne compte pas, donc **plancher exact** des allers-retours, étiquettes littérales uniquement (PGSSI-S). **Retiré** : l'écriture en retour du contenu depuis le chemin de lecture, ajoutée par symétrie apparente avec `UpdateAttachmentAsync`. La symétrie était fausse — l'existence d'une ligne `MailContents` est le **marqueur d'enrichissement** (`GetEnrichedUidsAsync`, `TryResolveExistingMailAsync`, `GetCoverageCountsAsync`) : une ligne corps-seul faisait **écarter le mail de l'enrichissement** ⇒ CDA jamais décodé, aucun document médical, aucun rattachement patient, et `NotifyAlreadyEnrichedAsync` annonçait au frontend que c'était fini. Perte de contenu clinique **silencieuse**, invisible pour 3 399 tests verts (aucun ne traverse ouverture → enrichissement). Défaut trouvé en **relecture humaine avant merge**. **Diagnostic de la US invalidé par le décompte lui-même** : `journey` n'appelle jamais `enrich` et chauffe par `getEmailContent`, donc l'étape 3 « servi base » mesurait des mails **jamais enrichis** — d'où 440 ≈ 442 ms. Verdict « étape 3 au rouge » **non opposable**. Garde-fous posés (avertissements dans `IMailRepository` et la doc de `GetEmailContentAsync`, test d'intégration asserant qu'une lecture n'écrit rien). **Arbitrage PO ouvert** : `questions/task-222.md`. | — |

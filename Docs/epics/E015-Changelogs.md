...
### v1.19 — Le chemin d'ouverture lisait le stock sans jamais l'écrire : 440 ms repayés à chaque relecture — task-222

- **Task** : task-222 — `done`. **PR** : `api-mail` #150 (label `awaiting-human-merge`). Commits `458fc48` (décompte des sollicitations), `317a403` (écriture en retour + 12 tests), `ee095f1` (simplify), `1c2b24c` (S125), `76ba6d3` (fenêtre TOCTOU consignée). `dtos-mss` : branche créée par `/start`, **aucun commit, aucune PR** — pas de changement de contrat, donc **pas de publish NuGet**.
- **Le constat, et sa provenance** : campagne de certification K=1 du 2026-08-03 (200 médecins, rythme réel, **6 343 ouvertures**), rapport `reports/2026-08-03/report-journey-certif-n200-180029.md`. Étape 3 « ouvrir un message enrichi (servi base) » : **p50 440 ms pour une cible de 100**. Étape 4 « message froid » : 442 ms pour 800 (✅). Étape 7 « PJ ~124 Ko du même message » : **34 ms** pour 500 (✅). Coût **invariant à la charge** : 439 / 443 / 440 ms aux paliers 50 / 100 / 200.
- **Ce que la campagne avait déjà établi, et qu'il ne fallait pas re-dériver** : sur la trace `4d911c462694fab4d7454de2453bb13f`, 19 ms pour résoudre le praticien et sa base, **420 ms à l'intérieur du verrou de session IMAP** pris **inconditionnellement** par `GetEmailContentAsync` (`ImapService.cs:1991`) avec **`WaitTimeMs=0`** — du travail, pas une file (contrairement à task-211) ; p95 serveur (497 ms) = p95 client (494 ms), rien hors application.
- **Ce qui manquait pour clore, et qui faisait partie de la US** : le **décompte des sollicitations du serveur par requête**. 420 ms est *compatible* avec quatre allers-retours à 95 ms sans le prouver, et les commandes IMAP n'étaient pas instrumentées à ce grain. `IMailServerSolicitationRecorder` (`Scoped`) + `MailServerSolicitationRecorder` (`ConcurrentQueue`) : compteur `mssante_mail_server_solicitations_total{command,operation}`, étiquette de trace `mss.mail_server.solicitations` apposée par `GetEmailContentAsync` avec le delta **propre à l'appel** (et non le cumul de la requête). Sites : `ResolveFolder` / `OpenFolder` / `FetchBodyStructure` / `FetchBodyPart` ×n / `CloseFolder` dans `ImapService`, `Connect` / `Authenticate` dans `ImapConnectionService` — **une session reprise du pool ne compte pas**, donc le décompte est une **borne inférieure exacte** des allers-retours, pas une estimation. **PGSSI-S** : les deux étiquettes sont des littéraux (`MailServerCommands`, familles d'opération) — même règle que `LockOperationFamily` (task-213), aucun chemin de dossier, aucun UID, aucun nom de PJ.
- **La cause, close — et ce n'est pas quatre allers-retours de trop** : `GetEmailContentAsync` **lisait** le stock (`mailRepository.GetMailAsync(..., WithContent)`, ex-ligne 1982) sans jamais **l'écrire**. Le corps ramené d'IMAP était assaini, renvoyé, puis **jeté** : aucune ligne `MailContents` n'était créée, le mail ne devenait jamais « enrichi », le garde `Content: not null` échouait à l'ouverture suivante, le verrou était repris et le trajet repayé. Les trois faits du constat en découlent : **il n'y avait pas de chaud** (440 ≈ 442) ; **la PJ, elle, a son write-back** (`GetAttachmentAsync` → `UpdateAttachmentAsync`, `GetAttachmentStreamAsync` → `BuildCachedAttachmentStreamAsync`), d'où 34 ms ; **un trajet par ouverture est un coût fixe**, d'où l'invariance à la charge.
- **Le harnais disait vrai sans l'être** : `warmUpOwnMailbox` (`tests/loadtest-k6/scenarios/journey.js`) chauffe la bande chaude par `getEmailContent` en commentant « le GET contenu matérialise le `MailContent` ». C'était **faux** — l'étape 3 mesurait du froid en croyant mesurer du chaud. C'est vrai depuis, **sans édition du harnais**. Corollaire à signaler : l'avertissement sur le partage de seed avec un tir `enrich`/`mixed` s'élargit, la bande chaude matérialisant désormais elle aussi du contenu.
- **Le correctif, à l'altitude que la cause désigne** : `IMailRepository.SaveMailContentAsync(folderPath, uid, content, ct) → bool`, symétrie exacte d'`UpdateAttachmentAsync`. Quatre clauses, chacune un garde-fou : (1) identité `(dossier, uid, génération)` — même filtre `UidValidity` que `GetMailAsync` (task-179), sinon on écrirait sur une ligne que la lecture ne sert jamais ; (2) **n'invente jamais de ligne `Mail`** → cloisonnement praticien (point de vigilance de la US) : un contenu ne peut pas être greffé sur une base qui ne porte pas le message ; (3) idempotente — n'écrase pas le contenu de la chaîne d'enrichissement, plus riche (résumé, embedding, `MailMedicalDocuments`) ; (4) refuse un corps vide des deux côtés, sinon elle fabriquerait l'état « présent mais inexploitable » que la lecture rejette, donc une boucle. Appel via `StoreFetchedContentAsync`, **best-effort** (`catch (Exception) when (ex is not OperationCanceledException)` → Warning) et **hors du verrou de session** : la détention n'est pas allongée, ce qui importe puisque c'est la grandeur que la campagne pointait.
- **Effet de bord assumé, et c'est une correction** : `HasDisplayableContent` remplace `existingMail is { Content: not null }`. Une ligne `MailContents` **entièrement vide** (promotion d'en-tête interrompue, cf. `TryResolveExistingMailAsync`) était servie telle quelle → **écran blanc définitif**, rien ne repassant jamais dessus. Le prédicat inclut `Summary` et `MedicalDocuments` **à dessein** : un mail dont toute la valeur clinique est dans son CDA a souvent un corps vide, et le re-solliciter le renverrait **sans** ses documents médicaux (le chemin IMAP ne les porte pas) — on n'échange pas du contenu clinique contre un corps vide. Contrepartie : un mail réellement vide côté serveur reste re-fetché à chaque ouverture (cas dégénéré, correction préférée à la vitesse).
- **Contenu périmé côté serveur — tranché** (exigé au DOD) : **impossible.** RFC 3501 — le corps d'un message d'UID donné est **immuable** dans une génération de dossier ; seuls les flags changent, un contenu différent est un autre UID. Sur renumérotation (`UIDVALIDITY`), le filtre de génération de task-179 cesse de servir la ligne périmée et le contenu est re-sollicité. L'écriture en retour **n'ouvre aucune fenêtre de péremption**. Couvert par `SaveMailContentAsync_WhenMailBelongsToAPreviousGeneration_WritesNothingAsync`.
- **Tests** : **12 ajoutés**, assertions sur un **nombre de sollicitations**, jamais sur un temps. `Services/Imap/ImapServiceTests.cs` (+6) — stock exploitable ⇒ `Assert.Empty(Commands)` **et** verrou de session non acquis **et** `ConnectInternalAsync` non appelé ; contenu absent ⇒ fetch **puis** `SaveMailContentAsync` reçu 1 fois avec le corps ramené ; ligne vide ⇒ re-fetch ; `MedicalDocuments` seuls ⇒ servi sans sollicitation (garde anti-régression clinique) ; write-back en exception ⇒ contenu tout de même rendu ; message disparu du serveur ⇒ **rien n'est écrit**. `Repository/MailRepositoryContentWriteBackTests.cs` (5, InMemory) — une clause du contrat par test. `Services/MailContentServedFromStoreIntegrationTests.cs` (3, **vrai PostgreSQL** via `PostgreSqlFixture`, vrai `MailRepository`) — dont la **preuve de bout en bout** : 1ʳᵉ ouverture ⇒ sollicite + stocke, **2ᵈᵉ ouverture (contexte et dépôt neufs) ⇒ `Assert.Equal(0, Count)`**.
- **Build / tests** : 0 erreur, 0 avertissement ; **3 399 verts**, 16 ignorés. **1 flaky pré-existant confirmé tel** : `MailExportServiceTests.BuildPdfWithoutAttachmentsOmitsAttachmentSection` a échoué sur **un** run Release complet, passe isolé en Release, et le run Release suivant est vert à 3 399/0 — `Services/Export` **hors diff** (même famille `UglyToad.PdfPig` que celle documentée en v1.18). À noter pour la reproductibilité : `--artifacts-path` casse `SecretLiteralScanTests` (`RepoRoot()` remonte chercher le `.sln` depuis `AppContext.BaseDirectory`) — artefact d'outillage, vert en sortie de build par défaut.
- **Simplify** : `MailServerSolicitationRecorder` passé de `lock` + `List` + champ de garde à `ConcurrentQueue` (trois membres en expressions) ; `SaveMailContentAsync` lit l'existence du contenu **dans la même requête** que le message via la projection `Select(m => new { m.Id, ContentCount = m.MailContents.Count })` — **l'idiome de `TryResolveExistingMailAsync` juste au-dessus**, 2 requêtes au lieu de 3.
- **Sonar** : **zéro dette introduite**. Itération 1 → **1** finding new-code attribuable : `csharpsquid:S125` (`IMailRepository.cs:66`), **faux positif de forme** — le bloc `//` documentant le contrat citait `(folderPath, uid)` / `MailContent` / `GetMailAsync` et ponctuait ses puces en `;`. Converti en doc XML (`<summary>`/`<remarks>`/`<returns>` + `<list>`), forme attendue sur un membre d'interface et déjà référencée par `<see cref>` depuis l'implémentation. Itération 2 → **0**. QG `ERROR` avant comme après sur 18 `new_violations`, **aucune** dans un fichier du diff : 16 → **task-220** (`report.py`, `journey.js`, `journey-model.js`, dont l'unique bug `python:S1244` et l'unique hotspot `javascript:S2245`), 2 `S103` → task-185 / task-218. `coverage` 86,9 → **87,0 %**, `new_coverage` **87,0 %**, duplication 0,5 %, ratings C / A / A inchangés. **Convention apprise** : entrée `S125` ajoutée à `conventions/csharp.md` (fichier **git-ignoré**, donc local au poste — non commitable).
- **Vérification demandée par la US sur `mssante_lock_hold_duration_seconds`** : la US soupçonnait « la même famille de défaut que task-214 ». **Ce n'en est pas une.** L'émission est correcte — `ImapLockScope.DisposeAsync` appelle `RecordLockHold(..., _tags.Family, _tags.Lane)` depuis task-214 — et `report.py` interroge bien `histogram_quantile(0.95, sum by (le, operation) (rate(mssante_lock_hold_duration_seconds_bucket{lock="imap_session"}[1m])))`. Le silence sur la fenêtre du tir est côté **observation** (fenêtre `[1m]` face à 3 111 itérations sur 35 min de parcours), pas côté instrumentation. Hors scope, à confirmer au prochain tir.
- **Signalé hors périmètre** : (1) **fenêtre TOCTOU sur l'écriture en retour** — `IX_MailContents_MailId` **n'est pas unique**, donc deux ouvertures froides concurrentes du même mail par deux `ClientSessionId` du même praticien (le verrou de session ne les exclut pas) peuvent insérer deux lignes. Conséquence **bornée** : corps identique (immuabilité de l'UID), `PopulateMailContentAsync` prend la dernière, aucune donnée fausse, aucune exception, cascade delete avec le mail ; la voie d'enrichissement porte la même fenêtre (documentée par `TryResolveExistingMailAsync`). Supprimer la fenêtre exige un **index unique donc une migration avec dé-doublonnage des lignes existantes** — task dédiée, pas un ajout discret dans un correctif de performance, et aucun test de cette PR n'exercerait la concurrence réelle. Consignée en commentaire (`76ba6d3`). (2) Le write-back stocke le HTML **assaini** ; conforme à l'intention de task-088 (assainissement au stockage *et* à la lecture, idempotent), signalé pour lever l'ambiguïté.
- **Non touché délibérément** : la suppression / le marquage comme lu (étape 8, 807 ms pour 200) — mêmes symptômes probables mais ces gestes **modifient** la boîte et doivent solliciter le serveur au moins une fois : plancher et arbitrage distincts, re-mesure après ce correctif (hors scope explicite de la US). L'envoi (étape 6) — task-216 va déplacer ce chemin. La grille SLO — validée par l'humain. L'outillage de mesure — task-224.
- **Reste dû** : les **deux critères de banc du DOD**, non exécutables par la forge et **à la charge de la recette**. (1) Le tir `journey` **K=1** au même palier de 200 médecins exige le nœud distant `MSS_LOADTEST_MAIL_HOST` et 35 min au rythme réel (`JOURNEY_TIME_COMPRESSION=1`, seule forme opposable). (2) La comparaison étape par étape est en outre **impossible côté forge** : `tests/loadtest-k6/reports/` est git-ignoré (seul `INDEX.md` suivi) et ne contient localement que `2026-07-25/26/27` — `INDEX.md` porte bien la ligne du tir du 2026-08-03 (200 VU, PASS) mais `report-journey-certif-n200-180029.md` **n'est pas sur la machine**. La causalité est en revanche **prouvée par test** (2ᵈᵉ ouverture ⇒ 0 sollicitation, l'énoncé même du DOD) ; le banc reste nécessaire pour **chiffrer** le gain et confirmer la non-régression des 7 autres étapes, en particulier l'étape 4 qui ne doit pas être déshabillée pour habiller l'étape 3.
...
| task-222 | **Le chemin d'ouverture lisait le stock sans jamais l'écrire.** `GetEmailContentAsync` interrogeait `MailContents` puis, en cas d'absence, ramenait le corps d'IMAP, l'assainissait, le renvoyait — et le **jetait**. Le mail ne devenait donc jamais « enrichi », le garde `Content: not null` échouait à l'ouverture suivante, le verrou de session était repris et le trajet repayé : **440 ms de p50 sur l'étape 3 pour une cible de 100**, contre 442 ms pour un message **froid** (donc aucun bénéfice à l'analyse préalable), et **invariant à la charge** (439/443/440 ms à 50/100/200) — un coût fixe, pas de la contention. Le contre-exemple était dans la même campagne : la PJ ~124 Ko du même mail en **34 ms**, parce que `GetAttachmentAsync`/`GetAttachmentStreamAsync` **écrivent en retour** (`UpdateAttachmentAsync`). Correctif : `SaveMailContentAsync`, symétrie exacte, appelée **hors du verrou de session** (la détention n'est pas allongée) et **best-effort** ; quatre clauses de contrat, chacune un garde-fou — identité `(dossier, uid, génération)` alignée sur `GetMailAsync` (task-179), **n'invente jamais de ligne `Mail`** (cloisonnement praticien), idempotente (ne dégrade pas le contenu enrichi : résumé, embedding, documents médicaux), refuse un corps vide des deux côtés (sinon boucle). **Contenu périmé : impossible** — RFC 3501, le corps d'un UID donné est immuable dans une génération, et une renumérotation (`UIDVALIDITY`) fait re-solliciter. Effet de bord favorable : une ligne de contenu **vide** (promotion d'en-tête interrompue) n'est plus servie — elle produisait un **écran blanc définitif**, rien ne repassant jamais dessus ; `Summary` et `MedicalDocuments` comptent dans le prédicat, sinon un mail dont la valeur clinique est dans son CDA serait re-fetché **sans** ses documents. **L'instrumentation manquante fait partie de la livraison** : `IMailServerSolicitationRecorder` (`Scoped`), compteur `mssante_mail_server_solicitations_total{command,operation}` + étiquette de trace `mss.mail_server.solicitations` — une session reprise du pool ne compte pas, donc borne inférieure **exacte** des allers-retours ; étiquettes littérales uniquement (PGSSI-S). 12 tests, dont 3 sur **vrai PostgreSQL** prouvant que la **2ᵈᵉ** ouverture sollicite le serveur **zéro fois**. Le harnais `journey` supposait à tort que relire matérialisait le contenu : **c'est vrai depuis, sans l'éditer**. Soupçon de la US sur `mssante_lock_hold_duration_seconds` **levé** : l'émission est correcte depuis task-214, le silence est côté observation. Sonar : **zéro dette introduite** (1 `S125`, faux positif de forme sur un bloc de commentaire, converti en doc XML). **Les deux critères de banc restent dus** — nœud distant `MSS_LOADTEST_MAIL_HOST` absent, et le rapport de référence du 2026-08-03 n'est pas sur la machine (`reports/` git-ignoré). | — |

...
- **Task** : task-222 — `wip` (re-cadrage PO en attente, cf. `questions/task-222.md`). **PR** : `api-mail` #150. Commits `458fc48` (décompte), `317a403` (correctif — **retiré**), `ee095f1` (simplify), `1c2b24c` (S125), `76ba6d3` (commentaire TOCTOU — retiré avec), `5da54bc` (**revert du correctif**). `dtos-mss` : branche vide, aucune PR, aucun publish NuGet.
...
- **Reste dû, et c'est un arbitrage PO** : (1) task-222 doit-elle être **fermée** (l'objectif est peut-être déjà atteint en production), **re-cadrée** sur « combien de sollicitations coûte réellement une relecture de message enrichi », ou maintenue ? (2) qui corrige le harnais pour que sa bande chaude soit réellement enrichie — task-224 ? (3) la lacune de couverture ouverture → enrichissement mérite-t-elle une US ? Détail et recommandations dans `questions/task-222.md`.

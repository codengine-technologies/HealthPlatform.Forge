...
| L'arrivée sur le tableau de bord ne fait plus la queue deux fois | La cause du coût de l'arrivée sur le tableau de bord est **établie par la mesure avant tout remède** : la lecture du dossier attendait deux fois son tour d'accès à la boîte, avec entre les deux une fenêtre où l'analyse des comptes-rendus pouvait s'intercaler plusieurs secondes. Elle n'attend plus qu'**une** fois, et la liste des dossiers pré-arme la vérification de chacun d'eux — quel que soit l'ordre dans lequel l'écran émet ses demandes. Deux passages successifs du même praticien sont éprouvés par test : le second ne repaie aucun échange avec le serveur, et un message arrivé entre-temps reste visible dans les dix secondes | task-273 | 🟠 Livré — en recouvrement avec une livraison voisine (task-270), arbitrage d'intégration posé |
...
| L'arrivée sur le tableau de bord ne fait plus la queue deux fois | 🟠 Livré — en recouvrement avec une livraison voisine (task-270), arbitrage d'intégration posé | La cause est **établie par la mesure avant tout remède**, phase par phase : la lecture du dossier attendait **deux fois** son tour d'accès à la boîte du praticien, avec entre les deux une fenêtre où l'analyse des comptes-rendus pouvait s'intercaler plusieurs secondes ; les échanges avec le serveur comptaient deux répétitions ; le stockage des dossiers, lui, était hors de cause. La lecture n'attend plus qu'**une** fois son tour, et la liste des dossiers pré-arme désormais la vérification de chacun d'eux — l'écran d'accueil peut émettre ses demandes dans n'importe quel ordre, la première qui passe sert les suivantes. Deux passages successifs du même praticien sont éprouvés par test : le second ne repaie **aucun** échange avec le serveur, et un message arrivé entre-temps reste visible dans les dix secondes — les fenêtres de fraîcheur existantes sont inchangées. Cette livraison recouvre en partie celle de la ligne précédente : l'intégration de l'une puis de l'autre est arbitrée avant tout merge | task-273 |
...
- v1.65 — **L'arrivée sur le tableau de bord ne fait plus la queue deux fois.** La cause du deuxième poste du temps serveur est établie par la mesure, phase par phase, avant tout remède : la lecture du dossier attendait deux fois son tour d'accès à la boîte, avec entre les deux une fenêtre où l'analyse pouvait s'intercaler plusieurs secondes. Elle n'attend plus qu'une fois, et la liste des dossiers pré-arme la vérification de chacun d'eux. Deux passages successifs sont éprouvés par test — le second ne repaie aucun échange avec le serveur, et un message arrivé entre-temps reste visible dans les dix secondes. Cette livraison recouvre en partie la v1.64 : l'ordre d'intégration des deux est arbitré avant tout merge. (task-273)

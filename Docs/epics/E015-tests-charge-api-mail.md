...
| Savoir où part la seconde et demie que coûte chaque envoi | L'envoi d'un message coûte ~1,3 s **quelle que soit la charge** — premier poste du temps serveur (25 %). Trois corrections l'ont visé sans le faire bouger, faute de savoir laquelle de ses étapes pèse. Chaque envoi est désormais décomposé en six étapes mesurées (contrôle d'opposition, construction du message, obtention de la session, transmission, archivage dans « Messages envoyés », reste) — le rapport de campagne dit lequel domine | task-260 | 🟢 Livré — mesure de confirmation à conduire au banc |
| La liste des messages ne fabrique plus ce qu'elle n'affiche pas | Afficher 25 en-têtes de messages fabriquait **298 éléments** en coulisse — corps complets des messages, résultats de biologie non affichés… soit **70 % de travail invisible**. La liste ne charge plus que ce qu'elle montre ; les pastilles (« biologie », « synthèse ») restent exactes, et **ce que le médecin voit est inchangé** — vérifié par test | task-261 | 🟢 Corrigé — mesure de confirmation à conduire au banc |
| L'écran d'accueil dit enfin ce qu'il demande au serveur de messagerie | L'appel le plus coûteux de l'arrivée sur le tableau de bord (65 % du coût de l'étape) échappait au décompte des sollicitations du serveur de messagerie : ses quatre allers-retours n'étaient comptés nulle part. Ils le sont désormais — la prochaine campagne pourra dire si un cache mieux réglé les évite, au lieu de le supposer | task-262 | 🟢 Livré — aucune réduction sans mesure, seuil de reprise posé |
...
| Le gain de l'écran d'accueil est vérifié sur le geste complet du praticien | Savoir que l'amélioration précédente tient **du point de vue du médecin**, et pas seulement pièce par pièce : deux arrivées successives sur le tableau de bord sont rejouées de bout en bout, et la seconde ne redemande **rien** au serveur de messagerie. La contrepartie est vérifiée dans le même mouvement — un message reçu entre les deux passages reste visible dans les dix secondes, donc l'écran ne montre jamais un état périmé sans le savoir | task-273 | 🟢 Livré — vérification de bout en bout, la cause du coût est documentée |
...
minutes redevient une gêne de quelques secondes.

La page d'état de santé, elle, **n'a pas été corrigée** — et c'est un tir de
vérification qui l'a établi, le lendemain. On avait branché son verdict sur
l'inventaire des connexions en mémoire ; en coupant volontairement le serveur de
messagerie sous six cents connexions, on a constaté qu'elle continuait
d'afficher « messagerie accessible ». La raison est instructive : quand la
connexion ne peut plus s'établir, l'application ne conserve **aucune** trace de
connexion — et le nettoyage des connexions mortes, celui-là même qui rend le
service à son état sain, **efface la preuve** que la page cherchait. Les deux ne
peuvent pas fonctionner ensemble. Le bon indicateur n'est pas ce que
...
## État de couverture (2026-09-18)

| Feature | Statut | Couverture | Tasks contributives |
|---|---|---|---|
| Le banc compte des praticiens inscrits, plus des robots en boucle | 🟢 Livré — premier tir `terrain` à 1 000 inscrits lancé le 2026-09-18, audit à venir | Jusqu'ici « mille médecins » désignait mille personnes ouvrant leur messagerie **toutes les 87 secondes, toutes en même temps, pendant trois heures et demie** (41 fois par heure chacune, mesuré). Le mode `terrain` modélise ce qui manquait : le praticien **s'absente** (il consulte), revient pour une session de quelques minutes, repart ; trois profils (occasionnel, régulier, intensif) attribués de façon rejouable. Le nombre de praticiens réellement présents **se déduit** au lieu d'être fixé à cent pour cent, et chaque verdict l'annonce (« N inscrits — M actifs en moyenne »). Les gestes et leurs seuils ne changent pas ; une mesure `terrain` ne se compare jamais à une mesure `journey`. Les profils sont des **hypothèses construites par cohérence**, remplaçables par les habitudes observées dans le journal d'audit une fois en production. Deux corrections d'instrument : la préparation d'un tir sur des boîtes déjà analysées ne consomme plus 78 % du temps de mesure pour ne rien faire (elle vérifie au lieu de préparer), et le débit publié par palier était sous-estimé d'un facteur 5,5 (rapporté à la fenêtre entière au lieu du seul régime) | task-321 |
...
| Le banc de mesure refuse de rendre un verdict sur des boîtes vides | 🟢 Corrigé et éprouvé — refus vérifié en conditions réelles | Les identifiants de messages ne sont jamais réutilisés par le serveur : après une purge, un re-semis ne repart pas de 1, et l'outil visait le vide en rendant des verts. Il sonde désormais **plusieurs** boîtes avant de tirer (elles peuvent différer entre elles) et refuse avec un message qui nomme la boîte, la cause et le geste. Éprouvé en reproduisant l'état fautif : refus exact sans décalage, mesure réelle avec | task-263 |
| Une mesure de capacité ne juge plus sa propre préparation | 🟢 Corrigé — confirmation à forte population à conduire | La préparation de chaque palier est calculée d'avance à partir du modèle (elle ne peut pas être observée en direct), marquée comme telle, et **exclue du verdict** — qui n'est porté que par le régime établi. Un palier dont la préparation avalerait toute la fenêtre est **refusé** avant de tirer. Contre-épreuve sur deux tirs identiques : les verdicts ne s'améliorent pas par construction — l'exclusion ne flatte pas les chiffres, elle les rend honnêtes | task-264 |
| Le banc peut enfin mesurer le coût des conversations | 🟢 Livré — campagnes de confirmation à conduire sur corpus fileté | Le corpus semé ne portait aucun en-tête de conversation. Conséquence exacte, et elle est mécanique : la requête qui cherche les messages citant un message d'origine ramenait **zéro ligne**, et la boucle de regroupement tournait sur rien. Le banc n'exerçait que la moitié bon marché du travail. **Une campagne aurait donc mesuré un gain modeste et l'aurait attribué à la faiblesse du correctif, alors qu'elle ne mesurait que son absence d'exercice.** Le seeder sait maintenant produire des conversations, en part réglable, de **longueurs variées** — un corpus de simples paires ferait passer un cas particulier pour la règle. **Le défaut reste le corpus d'hier, à l'identique** : l'activation est volontaire, pour qu'une campagne relancée sans y penser ne change pas de nature en silence. Le rapport publie la part semée, annonce que la taille moyenne de fil est **déduite** et non relevée, et — quand le corpus est fileté — **énonce la rupture de comparabilité** avec toutes les campagnes antérieures. Un témoin négatif est testé : sur corpus sans fil, aucune rupture n'est annoncée, sinon l'avertissement deviendrait du bruit. **Le critère qui juge la US est un fait, pas une intention** : sur une vraie base, avec le vrai générateur, le regroupement rend bien des conversations dont les tailles égalent les chaînes semées. Une première conception a d'ailleurs été **réfutée par son propre test** — des longueurs figées plafonnaient la part atteignable à 0,70, si bien qu'une part de 0,75 demandée n'aurait pas été tenue et que le rapport aurait publié un chiffre faux | task-267 |
...
| Envoyer un message n'attend plus la fin d'une analyse en cours | 🟠 **Retrait livré, confirmation au banc due** | À la réception d'un document, la messagerie l'analyse en tâche de fond — un travail de plusieurs secondes. Pendant ce temps, **toutes** les autres opérations sur la boîte du praticien passaient derrière, y compris le classement du message qu'il vient d'envoyer. Résultat mesuré : un envoi sur vingt dépassait **trente secondes**, le pire cas frôlant la minute. La boîte dispose désormais d'un **second accès réservé aux écritures** : classer un message envoyé, ou enregistrer un brouillon, ne fait plus la queue derrière l'analyse en cours. La règle qui protège la boîte — une seule commande à la fois sur un même accès — est **conservée telle quelle** ; c'est un second accès qui est ouvert, pas une règle qui est levée. **La campagne de confirmation a été conduite le 1ᵉʳ août, et elle ne tranche pas.** Ce qu'elle établit : l'envoi s'améliore, mais modestement — le pire cas passe de 35 à 29 secondes, là où l'objectif fixé était de descendre sous 10 —, et cette amélioration est **payée par la consultation**, qui se dégrade de 60 à 80 %. Ce qu'elle n'établit pas : le second accès ouvre une deuxième connexion par praticien, soit 2 500 de plus sur le banc, et sur ce poste l'infrastructure de test partage le processeur du service — la dégradation observée peut donc venir du banc autant que du correctif. Ce qu'elle ne pouvait pas établir du tout : le compte rendu conclut « aucun archivage sur la période », or l'archivage a bien eu lieu — il n'était **pas mesuré**, dix-neuf des vingt points d'attente de la boîte échappant à l'instrument (task-214). **Reste dû** : réparer la mesure, puis rejouer. La reprise devra aussi démêler ce qui, dans le ralentissement observé, revient à cette attente et ce qui revient au passage de 200 à 500 praticiens — les deux effets sont encore confondus. **Suite, et fin : la mesure a été réparée puis rejouée, et elle tranche contre le correctif.** Trois campagnes à 500 médecins, paramètres identiques, ont comparé la boîte **avec** et **sans** ce second accès. Le second accès tient sa promesse — le classement du message envoyé n'attend plus. Mais **l'envoi vu du médecin est plus lent avec lui que sans** : sept secondes et demie sans, dix à douze secondes avec. Ce que le second accès retire à l'attente, il le repaie en ouverture de connexion. Et le défaut qui l'avait motivé — un envoi sur vingt au-dessus de trente secondes — **n'est plus reproductible** : il venait d'un plafond du banc de test, corrigé depuis. Le second accès est donc **retiré** : la boîte revient à une seule connexion par médecin. La mesure qui a tranché est conservée dans le code, avec sa réserve — l'infrastructure de test partage le processeur du service, et c'est l'écart entre les trois campagnes qui est opposable, pas leur valeur absolue. **Reste dû** : une campagne de confirmation à 500 médecins pour vérifier que l'envoi repasse sous les dix secondes attendues | task-213, task-214, task-215, task-216 |
| L'envoi ne repaie plus une connexion à chaque message | 🟡 Livré, en attente d'intégration | L'envoi est resté la seule action hors des temps attendus à mille médecins (1,3 s au lieu d'une seconde), et son premier poste était connu : ouvrir une connexion d'envoi **à chaque message**, près d'une demi-seconde. Le mécanisme de réutilisation existait pourtant depuis longtemps — il ne servait jamais, et la mesure a dit pourquoi. La connexion gardée entre deux envois est entretenue par un battement régulier ; ce battement était réglé **exactement** sur le délai au-delà duquel le serveur coupe une connexion silencieuse. Chaque battement était donc une course perdue d'avance, et la connexion mourait sans laisser de trace : le journal ne disait rien, et chaque envoi repayait le prix fort en croyant réutiliser. Trois corrections : l'entretien bat désormais deux fois plus vite que le délai de coupure ; la connexion est conservée aussi longtemps que la session du praticien (au lieu d'être fermée par prudence entre deux envois espacés) ; et sa mort, quand elle survient, est désormais **écrite** dans le journal — l'aveuglement qui a coûté deux campagnes d'analyse ne peut plus se reproduire. Au passage, un aperçu du jeton d'authentification qui s'écrivait en clair dans les journaux à chaque connexion a été retiré. **Reste dû** : la campagne de confirmation (l'envoi doit repasser sous la seconde), et le second poste du coût de l'envoi — le classement dans « Envoyés » — est porté par une US dédiée | task-269 |
| L'envoi n'attend plus son classement dans « Envoyés » | 🟡 Livré, en attente d'intégration | Quand un praticien envoie un message, trois coûts se payaient l'un derrière l'autre avant qu'il reprenne la main : ouvrir la connexion d'envoi, transmettre, puis classer la copie dans « Envoyés » — ce dernier valant à lui seul près d'une demi-seconde. Or l'envoi est fait dès que le serveur de messagerie a accepté le message : la copie locale est un confort, pas une condition. Elle se fait désormais en coulisse, juste après, sur la même session que le reste des opérations du praticien. Si elle échoue, elle est retentée automatiquement (trois fois, espacées) ; un échec définitif est tracé et relié à l'envoi d'origine pour le support — jamais silencieux, et jamais présenté au praticien comme un échec d'envoi : ce quiproquo, qui poussait à renvoyer un document de santé déjà remis, est désormais impossible par construction. Limite assumée : si le service s'arrête brutalement dans les quelques secondes entre l'envoi et le classement, la copie peut manquer — le message, lui, est parti. Reste dû : la campagne de confirmation (avec la réutilisation de connexion déjà livrée, l'envoi est attendu bien sous la seconde) | task-272 |
...
- v1.53 — Chaque **envoi** est décomposé en six étapes mesurées. Son coût est fixe
  (1,3 s quelle que soit la charge, premier poste du temps serveur) et trois
  corrections l'avaient visé à l'aveugle ; le rapport nomme désormais l'étape qui
  domine. L'archivage absent se lit « non relevé », jamais zéro. (task-260)

- v1.54 — La **liste des messages ne fabrique plus ce qu'elle n'affiche pas** :
  70 % des éléments construits pour une page d'en-têtes n'étaient jamais montrés
  (corps complets, biologie non affichée). Le médecin voit exactement la même
  liste, les pastilles restent exactes, et il y a une requête de moins. (task-261)

- v1.55 — L'appel le plus coûteux de l'**écran d'accueil** (65 % du coût de
  l'étape) compte désormais ses allers-retours vers le serveur de messagerie —
  ils échappaient à tout décompte. Aucune optimisation sans mesure : le seuil qui
  rouvrira le sujet est écrit. Premier résultat de la US : sa demande initiale
  était déjà satisfaite depuis le 5 août, et le vérifier a évité un doublon. (task-262)

- v1.56 — Le **banc refuse de mentir** : viser des messages qui n'existent plus
  (identifiants jamais réutilisés après purge) rendait des verdicts verts sans
  rien mesurer — deux campagnes invalidées. Le tir refuse désormais de démarrer,
  en nommant la boîte fautive, la cause et le geste qui répare. (task-263)

- v1.57 — Une **mesure de capacité ne juge plus sa propre préparation** : la mise
  en condition (139 % de la fenêtre au dernier tir) est allouée d'avance, exclue
  du verdict, et le rapport dit quelle fenêtre porte chaque verdict — en signalant
  qu'un tir ancien n'est pas directement comparable. (task-264)

- v1.58 — **Le banc peut enfin mesurer le coût des conversations.** Le corpus d'essai n'en contenait aucune : le travail de regroupement en fils tournait à vide, et une campagne aurait conclu à un gain modeste faute de l'avoir exercé. Le corpus peut désormais en contenir, en part réglable et de longueurs variées ; le défaut reste le corpus d'hier, et le rapport énonce la rupture de comparabilité dès que le corpus est fileté. (task-267)
...
- v1.76 — **Une messagerie en panne cesse de passer pour une messagerie vide.** L'analyse des courriers — celle qui extrait les documents médicaux et les rattache au dossier du patient — pouvait s'arrêter net et ne jamais repartir, pendant que l'application répondait **« c'est fait »** à chaque demande. Mesuré à mille médecins : quatre-vingts minutes sans qu'un seul document soit traité, près d'un million d'autres demandes servies normalement pendant ce temps, et **264 praticiens sur 1 000** sans rien recevoir. Du point de vue du médecin, c'était indiscernable d'une boîte sans nouveaux documents. Trois défauts s'additionnaient : l'application **savait** que le serveur était injoignable mais jetait l'information, elle annonçait le nombre de messages **demandés** et non traités, et sa page d'état de santé affichait « accessible » alors qu'aucune connexion n'était vivante. Désormais l'analyse rend un compte rendu, une demande qui n'a rien pu lire **échoue franchement**, les connexions mortes sont rendues — une panne de quatre-vingts minutes redevient une gêne de quelques secondes — et le praticien voit un bandeau : **« Messagerie momentanément indisponible. Les documents seront rattachés dès son rétablissement. »** Il ne bloque rien, la liste reste affichée, il n'apparaît qu'une fois par épisode et disparaît au rétablissement. Les messages laissés de côté par une coupure sont repris automatiquement, un nombre de fois limité. **⚠️ La page d'état de santé, en revanche, n'a PAS été corrigée** : un tir de vérification avec coupure provoquée du serveur sous six cents connexions a montré qu'elle continue d'afficher « accessible » — le nettoyage des connexions mortes efface la preuve qu'elle cherchait, les deux ne pouvant pas fonctionner ensemble. Demande distincte. (task-315)
- v1.77 — **Le banc compte désormais des praticiens inscrits, plus des robots en boucle.** Jusqu'ici, « mille médecins » désignait mille personnes qui ouvraient leur messagerie **toutes les 87 secondes, toutes en même temps, pendant trois heures et demie** — quarante et une fois par heure chacune. Aucun praticien ne travaille ainsi : il consulte, revient quelques minutes sur sa messagerie, repart pour une demi-heure ou deux heures. Le nouveau mode `terrain` modélise **cette absence** et la variété des praticiens (occasionnel, régulier, intensif) ; le nombre de praticiens réellement présents à un instant donné **se déduit** au lieu d'être fixé à cent pour cent, et chaque verdict l'annonce : « mille inscrits — tant d'actifs en moyenne ». Les gestes et leurs seuils ne changent pas. Les profils sont pour l'instant des **hypothèses construites par cohérence** ; le journal d'audit de la plateforme permettra, une fois en production, de les remplacer par les habitudes réellement observées. Deux corrections d'instrument au passage : la préparation d'un tir sur des boîtes déjà analysées ne consomme plus les trois quarts du temps de mesure pour ne rien faire, et le débit publié pour un palier était **sous-estimé d'un facteur cinq et demi**. (task-321)

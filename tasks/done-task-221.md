...
- **Item 2** (sortie humaine, 2026-08-03) : `loadtest-dovecot-0` 1/1 Running, `loadtest-greenmail` 1/1 Running, `loadtest-toxiproxy` 1/1 Running, `pvc-nfs-loadtest` **Bound** (pv-nfs-loadtest, 50 Gi, RWX, classe nfs-loadtest). Note : GreenMail et Toxiproxy ont redémarré une fois ~7 min après création (probes serrées au premier pull d'image) — sans récidive sur 2 h.

...
| 2d   | 2     | duplication services (test-first) : tests de caractérisation d'abord (EmailFlagService trace +/-FLAGS, EmailBuildingService BuildHeaderOnlyMailDto mapping, CDA SectionType) puis extract-method — EmailFlagService.TraceFlagChange, EmailBuildingService.MapHeaderFields, BiologyAckService.ValidateAndPersistAsync, CdaParsingService.AddProblems | EmailFlagService, EmailBuildingService, BiologyAckService, CdaParsingService (+3 fichiers de tests) | 4 clusters intra-fichier | 0 | OK | application 1460/1460 OK, CDA intégration 9/9 OK (1 échec préexistant UserContextEnricherMiddleware 3724 hash, sans rapport) | dup 1.2%→0.8%, dup_lines 486→339, blocs 30→22, fichiers 10→6, coverage 74.8% |
| 2e   | 2     | new finding + duplication IMAP (test-first) : (1) fix CA1822 MapTraceToDto→static ; (2) dedup intra-ImapFolderService (GetFolderStatus/Query → ExecuteFolderSearchAsync) et chaîne 4-catch self-healing ImapService (GetFolderStatus/Query → MapFolderAccessExceptionAsync) avec 2 tests de caractérisation GetFolderAsync (not-found self-heal + erreur générique) ajoutés en amont ; (3) correctifs lint helper CA1068 (CancellationToken en dernier) + CA2254 (template log constant) | AuditTraceRepository, ImapFolderService, ImapService, ImapServiceTests | 1 finding (CA1822) + 2 clusters IMAP | 0 | OK | application IMAP 61/61 OK, infrastructure 346/346 OK (1 Export PDF flaky, sans rapport) | dup 0.8%→0.6%, dup_lines 339→253, blocs 22→18, fichiers 6, coverage 74.9%, code_smells 1→0, **findings 0** |
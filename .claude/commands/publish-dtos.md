...
1. Read the current version from `Dtos/Mss/HealthPlatform.Dtos.Mss.csproj`
2. Increment the major version (X.0.0 → X+1.0.0), or use the version passed as argument
3. Update the `<Version>` in the .csproj
4. Build and pack: `dotnet pack Dtos/Mss --configuration Release --output Dtos/Mss/artifacts -p:Version={version}`
5. Publish: `dotnet nuget push Dtos/Mss/artifacts/*.nupkg --api-key $GITHUB_TOKEN --source https://nuget.pkg.github.com/codengine-technologies/index.json --skip-duplicate`
...
8. Commit and push the DTO repo:
   ```bash
   cd Dtos/Mss && git add -A && git commit -m "feat(dtos): bump to {version}" && git push
   ```
